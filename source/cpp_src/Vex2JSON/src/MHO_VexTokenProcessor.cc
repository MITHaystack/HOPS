...
mho_json MHO_VexTokenProcessor::ProcessInt(const std::string& element_name, mho_json& format,
...
mho_json MHO_VexTokenProcessor::ProcessListInt(const std::string& element_name, mho_json& format,
                                               std::vector< std::string >& tokens)
{
...
mho_json MHO_VexTokenProcessor::ProcessListString(const std::string& element_name, mho_json& format,
                                                  std::vector< std::string >& tokens)
{
    mho_json element_data;
...
mho_json MHO_VexTokenProcessor::ProcessReal(const std::string& element_name, mho_json& format,
                                            std::vector< std::string >& tokens)
{
    mho_json element_data;
    ...
    if(ContainsWhitespace(tokens[0])) //if the value has units (we need to parse them out)
    {
        ...
        {
            msg_error("vex", "could not parse parameter: " << element_name << ", as (numerical) value from: <" << tokens[0]
                                                           << ">." << eom);
        }
    }
    else
...
mho_json MHO_VexTokenProcessor::ProcessListReal(const std::string& element_name, mho_json& format,
                                                std::vector< std::string >& tokens)
{
    mho_json element_data;
    ...
    for(std::size_t i = 0; i < tokens.size(); i++)
    {
        if(ContainsWhitespace(tokens[i])) //if the value has units (we need to parse them out)
        {
            ...
            {
                msg_error("vex", "could not parse parameter: " << element_name << ", as (numerical) value from: <" << tokens[i]
                                                               << ">." << eom);
            }
        }
        else
...
bool MHO_VexTokenProcessor::ContainsWhitespace(std::string value)
{
