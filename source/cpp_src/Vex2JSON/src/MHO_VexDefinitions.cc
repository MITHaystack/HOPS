...
void MHO_VexDefinitions::SetVexVersion(std::string version)
{
    ...
    else if(version.find("2.0") != std::string::npos)
    {
        fVexVersion = "2.0";
    }
    else if(version.find("ovex") != std::string::npos)
    {
        fVexVersion = "ovex";
    }
    else
...
std::string MHO_VexDefinitions::DetermineFileVersion(std::string filename)
{
    ...
    if(vfile.is_open())
    {
        ...
        if(contents.find(VexRevisionFlag()) != std::string::npos) //check for vex
        {
            ...
            if(start_pos != std::string::npos && end_pos != std::string::npos)
            {
                ...
                if(rev.find("1.5") != std::string::npos)
                {
                    revision = "1.5";
                }
                else if(rev.find("2.0") != std::string::npos)
                {
                    revision = "2.0";
                }
                else
...
vex_element_type MHO_VexDefinitions::DetermineType(std::string etype)
{
    if(etype == "int")
    {
        return vex_int_type;
    ...
    if(etype == "list_int")
    {
        return vex_list_int_type;
    }
    if(etype == "real")
    {
        return vex_real_type;
    }
    if(etype == "list_real")
    {
        return vex_list_real_type;
    }
    if(etype == "epoch")
    {
        return vex_epoch_type;
    }
    if(etype == "ra")
    {
        return vex_radec_type;
    }
    if(etype == "dec")
    {
        return vex_radec_type;
    ...
    if(etype == "string")
    {
        return vex_string_type;
    }
    if(etype == "list_string")
    {
        return vex_list_string_type;
    }
    if(etype == "compound")
    {
        return vex_compound_type;
    }
    if(etype == "list_compound")
    {
        return vex_list_compound_type;
    }
    if(etype == "keyword")
    {
        return vex_keyword_type;
    }
    if(etype == "reference")
    {
        return vex_reference_type;
    }
    if(etype == "link")
    {
        return vex_link_type;
    }
    return vex_unknown_type;
}

bool MHO_VexDefinitions::IsOptionalField(std::string& field_name)
{
    if(field_name.find_first_of(OptionalFlag()) != std::string::npos)
    {
        return true;
    }
    return false;
