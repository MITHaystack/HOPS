...
void MHO_VexParser::SetVexFile(std::string filename)
{
    fVexFileName = filename;
    DetermineFileVersion();
}

void MHO_VexParser::DetermineFileVersion()
{
    std::string rev = MHO_VexDefinitions::DetermineFileVersion(fVexFileName);
    SetVexVersion(rev);
}

mho_json MHO_VexParser::ParseVex()
{
...
void MHO_VexParser::RemoveComments()
{
...
void MHO_VexParser::MarkLiterals()
{
...
void MHO_VexParser::SplitStatements()
{
    //primitive search for start/end literal statements
...
void MHO_VexParser::IndexStatements()
{
            ...
            for(auto blk_it = fBlockNames.begin(); blk_it != fBlockNames.end(); blk_it++)
            {
                if(IsBlockStart(it->fContents, *blk_it))
                {
                    ...
                    else
                    {
                        ...
                        msg_error("vex", "duplicate " << *blk_it << " block within vex file: " << fVexFileName
                                                      << " found on line #" << it->fLineNumber << "." << eom);
                    }
                }
            }
        }
...
void MHO_VexParser::ProcessBlocks(mho_json& root)
{
    for(auto blk_it = fFoundBlocks.begin(); blk_it != fFoundBlocks.end(); blk_it++)
    {
...
std::vector< MHO_VexLine > MHO_VexParser::CollectBlockLines(std::string block_name)
{
...
bool MHO_VexParser::IsPotentialBlockStart(std::string line)
{
...
bool MHO_VexParser::IsBlockStart(std::string line, std::string blk_name)
{
...
void MHO_VexParser::SetVexVersion(std::string version)
{
    fVexVersion = version;
