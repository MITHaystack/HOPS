...
bool MHO_WeightChannelizer::InitializeImpl(const uch_weight_store_type* in, weight_store_type* out)
{
    fInitialized = false;
...
bool MHO_WeightChannelizer::ExecuteImpl(const uch_weight_store_type* in, weight_store_type* out)
{
    if(fInitialized)
    {
        auto* in_pp_axis = &(std::get< UCH_POLPROD_AXIS >(*(in)));
