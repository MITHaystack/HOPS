    ...
    if(fVis != nullptr && in != nullptr)
    {
        ...
        if(!pcal_start_ok)
        {
            msg_warn("calibration",
                     "no start time specified for pcal data, assuming it is the same as the visibilities" << eom);
        ...
        if(ap_delta > fAPEps)
        {
            msg_error("calibration",
                      "pcal accumulation period and visibility accumulation period are different by: " << ap_delta << eom);
            return false;
        }
        ...
        if(std::fabs(start_tdiff) < fStartEps * vis_tdelta && vis_ntime == pcal_ntime)
        {
            msg_debug("calibration",
                      "pcal accumulation period and start time coincide within tolerance, no need to trim pcal data" << eom);
            msg_debug("calibration", "start time delta is: " << start_tdiff << ", num pcal points: " << pcal_ntime
                                                             << ", num vis points: " << vis_ntime << eom);
            return true;
        }
        else
        {
            msg_debug("calibration", "need to trim pcal data to match visibilities, start time delta is: "
                                         << start_tdiff << ", num pcal points: " << pcal_ntime
                                         << ", num vis points: " << vis_ntime << eom);
        }
        ...
        msg_debug("calibration", "trimmed pcal, original start: " << pcal_start << ", new start: " << new_pcal_start
                                                                  << " for station: " << station_code << eom);
        msg_debug("calibration", "trimmed pcal, original number of APs: " << pcal_ntime << ", new number of APs: "
                                                                          << in->GetDimension(MTPCAL_TIME_AXIS) << eom);
