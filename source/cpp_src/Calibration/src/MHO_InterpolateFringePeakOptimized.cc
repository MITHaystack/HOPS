...
MHO_InterpolateFringePeakOptimized::MHO_InterpolateFringePeakOptimized()
{
    fMBDMaxBin = 0;
    fDRMaxBin = 0;
    fSBDMaxBin = 0;
...
bool MHO_InterpolateFringePeakOptimized::Initialize()
{
    ...
    if(fMBDAxis.GetSize() == 1)
    {
        return false;
    }
    if(fDRAxis.GetSize() == 1)
    {
        return false;
    }
...
bool MHO_InterpolateFringePeakOptimized::Execute()
{
    fine_peak_interpolation();
    return true;
}

void MHO_InterpolateFringePeakOptimized::SetMaxBins(int sbd_max, int mbd_max, int dr_max)
{
    fSBDMaxBin = sbd_max;
    fMBDMaxBin = mbd_max;
    fDRMaxBin = dr_max;
...
void MHO_InterpolateFringePeakOptimized::fine_peak_interpolation()
{
    profiler_scope();
    ...
    // Precompute AP time offsets (uniform grid, so recurrence is exact)
    std::vector< double > tdelta(nap);
    for(std::size_t ap = 0; ap < nap; ap++)
    {
        tdelta[ap] = ap_ax->at(ap) + ap_delta / 2.0 - fFRTOffset;
    }

    // Precompute channel sky frequencies
    std::vector< double > freq_arr(nchan);
    for(std::size_t fr = 0; fr < nchan; fr++)
    {
        freq_arr[fr] = (*chan_ax)(fr);
    }

    const std::complex< double > neg_two_pi_i(0.0, -2.0 * M_PI);

    // channel_phasor[imbd][fr] = exp(-2*pi*i * (mbd*(freq - ref_freq) + sideband_correction(mbd)))
    // Depends on imbd and fr only; shared across all isbd and idr iterations.
    std::vector< std::vector< std::complex< double > > > channel_phasor(5, std::vector< std::complex< double > >(nchan));
    for(int imbd = 0; imbd < 5; imbd++)
    {
        mbd = fMBDAxis.at(fMBDMaxBin) + 0.5 * (imbd - 2) * mbd_delta;
        double sb_corr = fRot.CalcSidebandCorrection(mbd);
        for(std::size_t fr = 0; fr < nchan; fr++)
        {
            channel_phasor[imbd][fr] = std::exp(neg_two_pi_i * (mbd * (freq_arr[fr] - fRefFreq) + sb_corr));
        }
    }

    // step_phasor[idr][fr]  = exp(-2*pi*i * freq * dr * ap_delta)  - advance by one AP
    // ap0_phasor[idr][fr]   = exp(-2*pi*i * freq * dr * tdelta[0]) - phasor at first AP
    // Both depend on idr and fr only; shared across all isbd and imbd iterations.
    std::vector< std::vector< std::complex< double > > > step_phasor(5, std::vector< std::complex< double > >(nchan));
    std::vector< std::vector< std::complex< double > > > ap0_phasor(5, std::vector< std::complex< double > >(nchan));
    for(int idr = 0; idr < 5; idr++)
    {
        dr = fDRAxis.at(fDRMaxBin) + 0.5 * (idr - 2) * dr_delta;
        for(std::size_t fr = 0; fr < nchan; fr++)
        {
    ...
    for(int isbd = 0; isbd < 5; isbd++)
    {
        for(int imbd = 0; imbd < 5; imbd++)
        {
            for(int idr = 0; idr < 5; idr++)
            {
                ...
                mbd_bin =
                    ((fMBDMaxBin + imbd - 2) % (int)fMBDAxis.GetSize() + (int)fMBDAxis.GetSize()) % (int)fMBDAxis.GetSize();
                ...
                //   theta = freq*dr*tdelta + mbd*(freq-ref_freq) + sideband_correction(mbd)
                //
                // The last two terms are constant per (imbd, fr) -> precomputed in channel_phasor.
                // The first term advances uniformly with ap -> use phasor recurrence (no exp() in loop).
                for(std::size_t fr = 0; fr < nchan; fr++)
                {
                    ...
                    // Combined initial phasor at ap=0: channel constant x first-AP rotation
                    std::complex< double > phasor = channel_phasor[imbd][fr] * ap0_phasor[idr][fr];
                    const std::complex< double >& step = step_phasor[idr][fr];

                    for(std::size_t ap = 0; ap < nap; ap++)
                    {
                        //std::cout<<"fr, ap ="<<fr<<", "<<ap<<" vrot input( "<<tdelta[ap]<<", "<<freq_arr[fr]<<", "<<fRefFreq<<", "<<dr<<", "<<mbd<<")"<<std::endl;
                        visibility_element_type vis = (*fSBDArray)(0, fr, ap, sbd_bin);
                        std::complex< double > x = vis * phasor;
                        x *= (*fWeights)(0, fr, ap, 0); //multiply by the 'weight'
                        z = z + x;
                        phasor *= step; // advance to next AP (replaces vrot call)
                    }
                }
                                       ...
                                       double* drfmax)                       // output: maximum value
{
    ...
    double dx0, dx1, dx2, center[3], x[3], x0_lower, x1_lower, x2_lower, x0_upper, x1_upper, x2_upper, value, bestval, xbest[3],
