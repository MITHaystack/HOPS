...
void MHO_DelayModel::ComputeModel()
{
    if(fRefData != nullptr && fRemData != nullptr)
    {
        ...
        msg_debug("calibration",
                  "reference station delay model start time is: " << hops_clock::to_iso8601_format(ref_start) << eom);
        msg_debug("calibration",
                  "remote station delay model start time is: " << hops_clock::to_iso8601_format(rem_start) << eom);
...
void MHO_DelayModel::CheckSplineInterval(int n_intervals, double tdiff, int& int_no, std::string station_id)
{
    if(n_intervals == 0)
    {
    ...
    if(tdiff < 0.0 || int_no < 0)
    {
        msg_warn("calibration",
                 "fourfit reference time is outside of station: " << station_id << " spline range - must extrapolate!" << eom);
        int_no = 0;
    }
