...
bool MHO_VisibilityChannelizer::InitializeImpl(const uch_visibility_store_type* in, visibility_store_type* out)
{
    fInitialized = false;
    if(in != nullptr && out != nullptr)
    {
        ...
        else
        {
            ...
            {
                msg_warn("calibration", "channel sizes are not a uniform number of spectral points, there are: "
                                            << channel_sizes.size() << " distinct sizes." << eom);
            }
...
bool MHO_VisibilityChannelizer::ExecuteImpl(const uch_visibility_store_type* in, visibility_store_type* out)
{
    if(fInitialized)
    {
        auto* in_pp_axis = &(std::get< UCH_POLPROD_AXIS >(*(in)));
