...
void compute_field_rotations_fixed(std::complex< double > cpolvalue[4], double* par_angle, double* elevation, int* mount_type)
{
    ...
    for(pp = 0; pp < 4; pp++)
    {
        ...
        {
            case POL_LL:
                radangle = (+1) * (par_angle[0] + elmult[mount_type[0]] * elevation[0]) +
...
int MHO_CircularFieldRotationCorrection::DetermineMountCode(const std::string& mount) const
{
    ...
    if(mount == "nasmythleft")
    {
        return NASMYTHLEFT;
    }
    if(mount == "nasmythright")
    {
        return NASMYTHRIGHT;
    }
...
std::complex< double > MHO_CircularFieldRotationCorrection::GetPrefactor(std::string pp_label)
{
    ...
    if(pp_label == "LL")
    {
        factor = pp_factors[POL_LL];
    }
    if(pp_label == "RR")
    {
        factor = pp_factors[POL_RR];
    }
    if(pp_label == "LR")
    {
        factor = pp_factors[POL_LR];
    }
    if(pp_label == "RL")
    {
        factor = pp_factors[POL_RL];
    }
