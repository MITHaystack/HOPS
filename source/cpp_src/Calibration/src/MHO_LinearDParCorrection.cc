...
bool MHO_LinearDParCorrection::ExecuteInPlace(visibility_type* in)
{
    PreMultiply(in);
    return true;
}
...
std::complex< double > MHO_LinearDParCorrection::GetPrefactor(std::string pp_label)
{
    TODO_FIXME_MSG(
        "TODO FIXME...we need to implement proper treatment of X/Y vs H/V pols. Fourfit has the convention inverted.")
