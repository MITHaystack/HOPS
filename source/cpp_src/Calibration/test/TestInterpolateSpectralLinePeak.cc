...
static void build_fixture(visibility_type& spec, weight_type& w, delay_rate_axis_type& dr_ax,
                          const double* dr_amps,   // NDR elements
                          const double* freq_amps, // NFREQ elements
                          double phi0, double peak_vis_scale,
...
static int configure_and_run(MHO_InterpolateSpectralLinePeak& op, visibility_type& spec, weight_type& w,
                             delay_rate_axis_type& dr_ax, int peak_chan, int peak_dr_bin, int peak_freq_bin,
                             double ref_freq_mhz)
{
...
static int test_initialize_guards()
{
           ...
           Verify via amplitude: amp == |peak_vis| / 1.0 = |peak_vis|. */
    {
        ...
        if(configure_and_run(op, spec, w, dr_ax, PEAK_CHAN, PEAK_DR_BIN, PEAK_FREQ_BIN, REF_FREQ_MHZ))
            return 1;
...
static int test_symmetric_peak()
{
    ...
    auto& chan_ax = std::get< CHANNEL_AXIS >(spec);
    auto& freq_ax = std::get< FREQ_AXIS >(spec);
    double expected_freq = chan_ax(static_cast< std::size_t >(PEAK_CHAN)) + freq_ax(static_cast< std::size_t >(PEAK_FREQ_BIN));
...
static int test_asymmetric_dr_offset()
{
    ...
    double expected_dr = dr_ax(static_cast< std::size_t >(PEAK_DR_BIN)) + target_offset * DR_BIN_W;
...
static int test_asymmetric_freq_offset()
{
    ...
    double expected_freq = chan_ax(static_cast< std::size_t >(PEAK_CHAN)) +
                           (freq_ax(static_cast< std::size_t >(PEAK_FREQ_BIN)) + target_offset * FREQ_BIN_W);
    REQUIRE(close(op.GetPeakSkyFrequencyMHz(), expected_freq, 1e-9));
...
static int test_fringe_quantities()
{
    ...
    REQUIRE(close(op.GetFringeRate(), expected_frate, std::fabs(expected_frate) * 1e-12));
    ...
    REQUIRE(close(op.GetPhaseDelay(), expected_pd, std::fabs(expected_pd) * 1e-12));
...
static int test_clamp_offset()
{
    ...
    /* clamped offset = +0.5 */
    double expected_dr = dr_ax(static_cast< std::size_t >(PEAK_DR_BIN)) + 0.5 * DR_BIN_W;
    REQUIRE(close(op.GetDelayRate(), expected_dr, 1e-15));
...
static int test_edge_wrap()
{
    ...
    if(configure_and_run(op, spec, w, dr_ax, PEAK_CHAN, 0, 0, REF_FREQ_MHZ))
        return 1;
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_initialize_guards())
        return 1;
    if(test_symmetric_peak())
        return 1;
    if(test_asymmetric_dr_offset())
        return 1;
    if(test_asymmetric_freq_offset())
        return 1;
    if(test_fringe_quantities())
        return 1;
    if(test_clamp_offset())
        return 1;
    if(test_flat_region())
        return 1;
    if(test_idempotency())
