    ...
    for(std::size_t pp = 0; pp < npol; pp++)
        for(std::size_t ch = 0; ch < nchan; ch++)
            for(std::size_t ap = 0; ap < nap; ap++)
                for(std::size_t sp = 0; sp < nspec; sp++)
                {
                    CHECK_CLOSE(vis(pp, ch, ap, sp).real(), after_first(pp, ch, ap, sp).real(), 1e-15);
                    CHECK_CLOSE(vis(pp, ch, ap, sp).imag(), after_first(pp, ch, ap, sp).imag(), 1e-15);
                }
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_lsb_dc_zero())
        return 1;
    if(test_all_pol_ap_coverage())
        return 1;
    if(test_missing_sideband_default())
        return 1;
    if(test_idempotency())
        return 1;
