...
static void fill_phase_ramp(visibility_type& vis, double rate_true, double ref_freq, const std::vector< double >& chan_freqs,
                            double time_delta)
{
...
static void set_axis_values(visibility_type& vis, const std::vector< double >& chan_freqs, double time_delta)
{
...
static int argmax_dr(const sbd_type& out, std::size_t pp, std::size_t ch, std::size_t fb, int fDRSPSize)
{
    double max_val = -1.0;
    int max_dr = 0;
...
static double peak_amp_dr(const sbd_type& out, std::size_t pp, std::size_t ch, std::size_t fb, int fDRSPSize)
{
    double max_val = 0.0;
    for(int dr = 0; dr < fDRSPSize; dr++)
    {
...
static int test_calculate_search_space_size()
{
    ...
    for(auto& tc : cases)
    {
        ...
        {
            std::cerr << "FAIL: CalculateSearchSpaceSize(" << tc.n << ") = " << result << ", expected " << tc.expected << " @ "
...
static int test_execute_peak_at_injected_rate()
{
    ...
    for(std::size_t ch = 0; ch < 4; ch++)
    {
        ...
        {
            std::cerr << "FAIL: argmax dr for channel " << ch << " = " << dr_idx << ", expected " << expected_dr << " @ "
                      << __FILE__ << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
    ...
    if(pa < 0.95 * 16.0)
    {
        std::cerr << "FAIL: peak amplitude " << pa << " < 0.95*16 = " << (0.95 * 16.0) << " @ " << __FILE__ << ":" << __LINE__
                  << std::endl;
        return 1;
    }
...
static int test_execute_delay_rate_axis_values()
{
    ...
    for(int dr = 0; dr < 32; dr++)
    {
        ...
        {
            std::cerr << "FAIL: TIME axis[dr=" << dr << "] = " << actual << ", expected " << expected << " @ " << __FILE__
                      << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
...
static int test_sign_flip_round_trip()
{
    ...
    /* Positive rate: peak at dr=24 */
    {
        ...
        if(dr_idx != 24)
        {
            std::cerr << "FAIL: positive-rate argmax = " << dr_idx << ", expected 24"
                      << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
    ...
    /* Negative rate: peak at dr=8  (dr = round(-0.25*32) + 16 = -8 + 16 = 8) */
    {
        ...
        if(dr_idx != 8)
        {
            std::cerr << "FAIL: negative-rate argmax = " << dr_idx << ", expected 8"
                      << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
...
static int test_weights_applied()
{
    ...
    double ref_peak = peak_amp_dr(out_full, 0, 0, 0, dr_full.GetDelayRateSearchSpaceSize());
    ...
    int dr_idx = argmax_dr(out_mask, 0, 0, 0, dr_mask.GetDelayRateSearchSpaceSize());
    if(dr_idx != 24)
    {
        std::cerr << "FAIL: weighted argmax = " << dr_idx << ", expected 24"
                  << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
        return 1;
    }
    ...
    double mask_peak = peak_amp_dr(out_mask, 0, 0, 0, dr_mask.GetDelayRateSearchSpaceSize());
    double ratio = mask_peak / ref_peak;
    if(ratio < 0.85 || ratio > 1.0)
    {
        std::cerr << "FAIL: peak ratio = " << ratio << " not in [0.85, 1.0] (ref=" << ref_peak << ", masked=" << mask_peak
                  << ")"
                  << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
        return 1;
    }
...
static int test_reinitialize_different_size()
{
    ...
    std::vector< double > chan_freqs32 = {8000.0, 8016.0, 8032.0, 8048.0, 8064.0, 8080.0, 8096.0, 8112.0,
                                          8128.0, 8144.0, 8160.0, 8176.0, 8192.0, 8208.0, 8224.0, 8240.0,
                                          8256.0, 8272.0, 8288.0, 8304.0, 8320.0, 8336.0, 8352.0, 8368.0,
                                          8384.0, 8400.0, 8416.0, 8432.0, 8448.0, 8464.0, 8480.0, 8496.0};
    ...
    /* First init: (1,4,16,4) -> fDRSPSize=32, np=128 */
    {
        ...
        if(dr_idx != 24)
        {
            std::cerr << "FAIL: reinit-first argmax = " << dr_idx << ", expected 24"
                      << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
    ...
    /* Second init: (1,32,32,4) -> fDRSPSize=64, np=256 */
    {
        ...
        if(dr_idx != 48)
        {
            std::cerr << "FAIL: reinit-second argmax = " << dr_idx << ", expected 48"
                      << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
...
static int test_channel_frequency_scaling()
{
    ...
    for(std::size_t ch = 0; ch < 4; ch++)
    {
        ...
        if(dr_idx != expected_dr)
        {
            std::cerr << "FAIL: channel " << ch << " (freq=" << chan_freqs[ch] << ") argmax = " << dr_idx << ", expected "
                      << expected_dr << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
            return 1;
        }
    }
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_calculate_search_space_size())
        return 1;
    if(test_initialize())
        return 1;
    if(test_execute_peak_at_injected_rate())
        return 1;
    if(test_execute_delay_rate_axis_values())
        return 1;
    if(test_sign_flip_round_trip())
        return 1;
    if(test_weights_applied())
        return 1;
    if(test_null_args())
        return 1;
    if(test_reinitialize_different_size())
        return 1;
    if(test_channel_frequency_scaling())
        return 1;
