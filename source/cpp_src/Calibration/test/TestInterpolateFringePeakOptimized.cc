...
static void inject_fringe(visibility_type& sbd, int sbd_bin, double dr0, double mbd0, double amp0, double phase0,
                          const channel_axis_type& chan_ax, const time_axis_type& ap_ax, double ref_freq, double frt_offset)
{
    ...
    for(std::size_t fr = 0; fr < nchan; fr++)
    {
        ...
        for(std::size_t ap = 0; ap < nap; ap++)
        {
            ...
            sbd(0, fr, ap, sbd_bin) = amp0 * std::conj(vr) * std::complex< double >(std::cos(phase0), std::sin(phase0));
        }
    }
}

static void build_fixture(visibility_type& sbd, weight_type& w, time_axis_type& mbd_ax, delay_rate_axis_type& dr_ax,
                          int& sbd_max, int& mbd_max, int& dr_max, double& ref_freq, double& frt_offset, double dr0,
                          double mbd0, double phase0, double amp0, double total_sw)
{
    ...
    inject_fringe(sbd, SBD_MAX, dr0, mbd0, amp0, phase0, chan_ax, ap_ax, REF_FREQ, FRT_OFFSET);
...
static int configure_and_run(MHO_InterpolateFringePeak& op, visibility_type& sbd, weight_type& w, time_axis_type& mbd_ax,
                             delay_rate_axis_type& dr_ax, int sbd_max, int mbd_max, int dr_max, double ref_freq,
                             double frt_offset)
{
    op.SetSBDArray(&sbd);
    op.SetWeights(&w);
...
static int configure_and_run(MHO_InterpolateFringePeakOptimized& op, visibility_type& sbd, weight_type& w,
                             time_axis_type& mbd_ax, delay_rate_axis_type& dr_ax, int sbd_max, int mbd_max, int dr_max,
...
static int cross_check(MHO_InterpolateFringePeak& ref, MHO_InterpolateFringePeakOptimized& opt)
{
...
static int test_elementwise_centered_peak()
{
    ...
    build_fixture(sbd, w, mbd_ax, dr_ax, sbd_max, mbd_max, dr_max, ref_freq, frt_offset, 0.0, 0.0, 0.0, 1.0,
                  (double)(NCH * NAP));

    MHO_InterpolateFringePeak ref_op;
    MHO_InterpolateFringePeakOptimized opt_op;

    if(configure_and_run(ref_op, sbd, w, mbd_ax, dr_ax, sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
    if(configure_and_run(opt_op, sbd, w, mbd_ax, dr_ax, sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
...
static int test_elementwise_offbin_peak()
{
    ...
    build_fixture(sbd, w, mbd_ax, dr_ax, sbd_max, mbd_max, dr_max, ref_freq, frt_offset, dr0, mbd0, 0.0, amp0,
                  (double)(NCH * NAP));
...
static int test_initialize_guards()
{
    ...
    dr_ax.Resize(NDR);
    op.SetDRAxis(&dr_ax);
    weight_type w_no_tag;
    w_no_tag.Resize(1, NCH, NAP, NSBD);
    for(int fr = 0; fr < NCH; fr++)
        for(int ap = 0; ap < NAP; ap++)
...
static int test_idempotency()
{
    ...
    build_fixture(sbd, w, mbd_ax, dr_ax, sbd_max, mbd_max, dr_max, ref_freq, frt_offset, dr0, mbd0, 0.0, 1.0,
                  (double)(NCH * NAP));

    MHO_InterpolateFringePeakOptimized op;
    if(configure_and_run(op, sbd, w, mbd_ax, dr_ax, sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
...
static int test_edgebin_wrap()
{
    ...
    if(configure_and_run(ref_op, sbd, w, mbd_ax, dr_ax, edge_sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
    if(configure_and_run(opt_op, sbd, w, mbd_ax, dr_ax, edge_sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_elementwise_centered_peak())
        return 1;
    if(test_elementwise_offbin_peak())
        return 1;
    if(test_initialize_guards())
        return 1;
    if(test_idempotency())
        return 1;
    if(test_edgebin_wrap())
        return 1;
    if(test_allzero_cube())
        return 1;
