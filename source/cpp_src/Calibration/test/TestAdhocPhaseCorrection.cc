...
static void write_adhoc_file(const std::string& path, const std::vector< std::vector< double > >& rows)
{
    ...
    if(!ofs.is_open())
    {
        std::cerr << "FAIL: cannot open adhoc file " << path << " @ " << __FILE__ << ":" << __LINE__ << std::endl;
        std::exit(1);
    }
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_case1_none_noop())
        return 1;
    if(test_case2_polynomial_c0())
        return 1;
    if(test_case3_polynomial_c1())
        return 1;
    if(test_case4_sinewave())
        return 1;
    if(test_case5_phyle_differential())
        return 1;
    if(test_case6_phyle_unknown_code())
        return 1;
    if(test_case7_missing_start())
        return 1;
    if(test_case8_too_few_aps())
        return 1;
