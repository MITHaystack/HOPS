...
static visibility_type make_vis(const std::vector< std::string >& pprods, const std::string& ref_mk4,
                                const std::string& ref_code, const std::string& rem_mk4, const std::string& rem_code)
{
    visibility_type vis;
...
static void run_relabeler(MHO_PolarizationProductRelabeler< visibility_type >& rel, visibility_type* vis)
{
    rel.SetArgs(vis);
    rel.Initialize();
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_ref_station_swap())
        return 1;
    if(test_rem_station_swap())
        return 1;
    if(test_both_stations_swap())
        return 1;
    if(test_no_match())
        return 1;
    if(test_wildcards())
        return 1;
    if(test_chars_outside_swap())
        return 1;
    if(test_multi_char_swap_rejected())
        return 1;
    if(test_mixed_pol_partial())
        return 1;
