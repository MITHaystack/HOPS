                             ...
                             double frt_offset)
{
    ...
    if(!op.Initialize())
    {
        std::cerr << "FAIL: op.Initialize() @ " << __FILE__ << ":" << __LINE__ << std::endl;
        return 1;
    }
    if(!op.Execute())
    {
        std::cerr << "FAIL: op.Execute() @ " << __FILE__ << ":" << __LINE__ << std::endl;
        return 1;
    }
    return 0;
}
...
static int test_centered_peak()
{
    ...
    REQUIRE(close(op.GetFringeRate(), expected_dr * ref_freq, 0.5 * DR_DELTA * ref_freq));
...
static int test_amplitude_weight_scaling()
{
    ...
    build_fixture(sbd1, w1, mbd_ax1, dr_ax1, sbd_max, mbd_max, dr_max, ref_freq, frt_offset, 0.0, 0.0, 0.0, 1.0,
                  (double)(NCH * NAP));

    MHO_InterpolateFringePeak op1;
    if(configure_and_run(op1, sbd1, w1, mbd_ax1, dr_ax1, sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
    ...
    build_fixture(sbd2, w2, mbd_ax2, dr_ax2, sbd_max, mbd_max, dr_max, ref_freq, frt_offset, 0.0, 0.0, 0.0, 1.0,
                  2.0 * (double)(NCH * NAP));

    MHO_InterpolateFringePeak op2;
    if(configure_and_run(op2, sbd2, w2, mbd_ax2, dr_ax2, sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
...
static int test_edgebin_wrap()
{
    ...
    MHO_InterpolateFringePeak op;
    if(configure_and_run(op, sbd, w, mbd_ax, dr_ax, edge_sbd_max, mbd_max, dr_max, ref_freq, frt_offset))
        return 1;
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_initialize_guards())
        return 1;
    if(test_centered_peak())
        return 1;
    if(test_offbin_peak())
        return 1;
    if(test_amplitude_weight_scaling())
        return 1;
    if(test_idempotency())
        return 1;
