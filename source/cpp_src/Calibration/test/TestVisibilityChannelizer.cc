...
static void build_fixture(uch_visibility_store_type& uch, std::size_t npp, std::size_t nap, std::size_t nchan,
                          std::size_t nspec, bool ascending_freq)
{
    ...
    for(std::size_t t = 0; t < nap; t++)
    {
        ...
        {
            uch(0, t, f) = std::complex< float >(static_cast< float >(f), static_cast< float >(t));
        }
    }
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    if(test_dimensions())
        return 1;
    if(test_data_placement())
        return 1;
    if(test_channel_labels())
        return 1;
    if(test_channel_sort_order())
        return 1;
    if(test_empty_input())
        return 1;
    if(test_idempotency())
