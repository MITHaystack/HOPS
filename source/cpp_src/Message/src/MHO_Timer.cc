...
timespec MHO_Timer::GetTimeDifference(const timespec& start, const timespec& stop) const
{
    timespec ret_val;
...
timespec MHO_Timer::GetDurationAsTimeSpec() const
{
    return GetTimeDifference(fStart, fStop);
}

double MHO_Timer::GetTimeSinceStart() const
{
    timespec tmp;
