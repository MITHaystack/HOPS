...
template< typename XValueType > inline void make_attribute(const std::string& key, XValueType value, hid_t parent_dataset_id)
{
...
template<> inline void make_attribute< std::string >(const std::string& key, std::string value, hid_t parent_dataset_id)
{
    if(H5Aexists(parent_dataset_id, key.c_str()) > 0)
    {
...
herr_t inline make_vector_attribute(const std::string& key, const std::vector< XDataType >* data, hid_t parent_dataset_id)
{
    herr_t status = -1;
    if(H5Aexists(parent_dataset_id, key.c_str()) > 0)
    {
...
template<>
herr_t inline make_vector_attribute(const std::string& key, const std::vector< std::string >* data, hid_t parent_dataset_id)
{
    herr_t status = -1;
    if(H5Aexists(parent_dataset_id, key.c_str()) > 0)
    {
