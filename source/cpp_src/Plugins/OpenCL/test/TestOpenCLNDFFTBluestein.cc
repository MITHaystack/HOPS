...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    for(unsigned int i = 0; i < NDIM; i++)
    {
        ...
        if(dim[i] > max_dim_size)
        {
            max_dim_size = dim[i];
        }
        if(fft_work[i].fM > max_work_size)
        {
            max_work_size = fft_work[i].fM;
        }
    }
    ...
    for(unsigned int i = 0; i < NDIM; i++)
    {
        ...
        auto fScaleBufferCL =
            new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY, max_dim_size * sizeof(CL_TYPE2));

        //buffer for the bluestein circulant vector
        auto fCirculantBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
...
#endif
        CL_ERROR_TRY
        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fScaleBufferCL, CL_TRUE, 0, N * sizeof(CL_TYPE2),
                                                                          s.GetData());
        CL_ERROR_CATCH
        ...
        CL_ERROR_TRY
        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fCirculantBufferCL, CL_TRUE, 0, M * sizeof(CL_TYPE2),
                                                                          c.GetData());
        CL_ERROR_CATCH
    ...
    unsigned int n_bytes = static_cast< unsigned int >(max_work_size * max_dim_size * max_dim_size *
                                                       sizeof(MHO_OpenCLTypeMap< std::complex< double > >::mapped_type));
    ...
    for(unsigned int D = 0; D < NDIM; D++)
    {
        ...
        std::cout << "D = " << D << " dim = " << dim[D] << " n local = " << fNLocal << " n_global " << n_global
                  << " ndummy = " << nDummy << std::endl;
        ...
        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fFFTKernel, cl::NullRange, global, local);
    }
