...
void MHO_OpenCLInterface::InitializeOpenCL()
{
    ...
    // the GPU
    cl_context_properties cps[3] = {CL_CONTEXT_PLATFORM, (cl_context_properties)(fPlatforms[HOPS_OPENCL_PLATFORM])(), 0};
...
void MHO_OpenCLInterface::SetGPU(unsigned int i)
{
    if(i >= fDevices.size())
    {
        msg_warn("opencl", "Cannot set GPU device to ID # " << i << ", since there are only " << fDevices.size()
                                                            << " devices available." << eom);
        return;
    }
#ifdef HOPS_USE_DOUBLE_PRECISION
    cl::STRING_CLASS extensions;
    fDevices[i].getInfo(CL_DEVICE_EXTENSIONS, &extensions);
    if((extensions.find("cl_khr_fp64") == std::string::npos) && (extensions.find("cl_amd_fp64") == std::string::npos))
    {
        msg_warn(
            "opencl",
            "Cannot set GPU device to ID # "
                << i
    ...
    msg_info("opencl", "Setting GPU device to ID # " << i << " (" << name << ") of " << fDevices.size()
                                                     << " available devices (double precision supported). " << eom);
#else
    cl::STRING_CLASS name;
    fDevices[i].getInfo(CL_DEVICE_NAME, &name);
    msg_info("opencl",
             "Setting GPU device to ID # " << i << " (" << name << ") of " << fDevices.size() << " available devices. " << eom);
