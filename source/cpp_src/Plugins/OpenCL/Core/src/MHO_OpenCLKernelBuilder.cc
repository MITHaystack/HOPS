...
cl::Kernel* MHO_OpenCLKernelBuilder::BuildKernel(std::string SourceFileName, std::string KernelName, std::string BuildFlags)
{
    ...
    catch(cl::Error error)
    {
        ...
        s << "Build Status: "
          << program.getBuildInfo< CL_PROGRAM_BUILD_STATUS >(MHO_OpenCLInterface::GetInstance()->GetDevice()) << std::endl;
        s << "Build Options:\t"
          << program.getBuildInfo< CL_PROGRAM_BUILD_OPTIONS >(MHO_OpenCLInterface::GetInstance()->GetDevice()) << std::endl;
        s << "Build Log:\t " << program.getBuildInfo< CL_PROGRAM_BUILD_LOG >(MHO_OpenCLInterface::GetInstance()->GetDevice())
          << std::endl;
    ...
    build_log_stream << program.getBuildInfo< CL_PROGRAM_BUILD_LOG >(MHO_OpenCLInterface::GetInstance()->GetDevice())
                     << std::endl;
    ...
    {
        std::cout << "Kernel construction of " << KernelName << " failed with error code: " << error.what() << ": "
                  << error.err() << ": " << MHO_OpenCLInterface::GetInstance()->GetErrorMessage(error.err()) << std::endl;
        if(build_log.size() != 0)
        {
            s << build_log;
            std::cout << s.str() << std::endl;
