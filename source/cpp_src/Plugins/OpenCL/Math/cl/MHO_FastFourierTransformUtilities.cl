...
void
#ifdef FFT_USE_CONST_MEM
PermuteArray(unsigned int N, __constant const unsigned int* permutation_index_arr, __global CL_TYPE2* arr)
#else
PermuteArray(unsigned int N, __global const unsigned int* permutation_index_arr, __global CL_TYPE2* arr)
#endif
{
    //expects an array of size N
    CL_TYPE2 val;
...
void
#ifdef FFT_USE_CONST_MEM
FFTRadixTwo_DIT(unsigned int N, __global CL_TYPE2* data, __constant const CL_TYPE2* twiddle)
#else
FFTRadixTwo_DIT(unsigned int N, __global CL_TYPE2* data, __global const CL_TYPE2* twiddle)
#endif
{
    //temporary workspace
    CL_TYPE2 H0;
...
void
#ifdef FFT_USE_CONST_MEM
FFTRadixTwo_DIF(unsigned int N, __global CL_TYPE2* data, __constant const CL_TYPE2* twiddle)
#else
FFTRadixTwo_DIF(unsigned int N, __global CL_TYPE2* data, __global const CL_TYPE2* twiddle)
#endif
{
    //temporary workspace
    CL_TYPE2 H0;
...
FFTBluestein(unsigned int N,
             ...
             __constant CL_TYPE2* scale, //must be size N
             __constant CL_TYPE2* circulant, //must be size M
             __global CL_TYPE2* data) //must be size M, initially filled up to N
#else
FFTBluestein(unsigned int N,
             unsigned int M,
             __global CL_TYPE2* twiddle, //must be size M
