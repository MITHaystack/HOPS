...
void MHO_OpenCLFastFourierTransformPlan::ConstructKernel(const std::string& file_name, const std::string& kernel_name,
                                                         const std::string& cflags)
{
    std::cout << "building file: " << kernel_name << " kernel: " << kernel_name << std::endl;
    //Get name of kernel source file
    std::stringstream clFile;
    clFile << MHO_OpenCLInterface::GetInstance()->GetKernelPath() << clFile << "/" << file_name;
...
void MHO_OpenCLFastFourierTransformPlan::BuildCodeList()
{
    // radix2_strided,
    ...
    fSourceCodeMap.insert(radix2_strided, std::make_tuple("MHO_MultidimensionalFastFourierTransformStrided_kernel.cl",
                                                          "MultidimensionalFastFourierTransformStrided_Radix2Stage", ""));

    fSourceCodeMap.insert(radix2_strided_const,
                          std::make_tuple("MHO_MultidimensionalFastFourierTransformStrided_kernel.cl",
                                          "MultidimensionalFastFourierTransformStrided_Radix2Stage", " -D FFT_USE_CONST_MEM "));

    fSourceCodeMap.insert(radix2_private, std::make_tuple("MHO_MultidimensionalFastFourierTransformPrivate_kernel.cl",
                                                          "MultidimensionalFastFourierTransformPrivate_Radix2Stage", ""));

    fSourceCodeMap.insert(radix2_private_const,
                          std::make_tuple("MHO_MultidimensionalFastFourierTransformPrivate_kernel.cl",
                                          "MultidimensionalFastFourierTransformPrivate_Radix2Stage", " -D FFT_USE_CONST_MEM "));
}
