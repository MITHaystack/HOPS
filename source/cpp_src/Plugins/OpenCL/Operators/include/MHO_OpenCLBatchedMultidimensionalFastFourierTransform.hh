...
template< typename XArgType > class MHO_OpenCLBatchedMultidimensionalFastFourierTransform: public MHO_Operator
{
    public:
        ...
        MHO_OpenCLBatchedMultidimensionalFastFourierTransform()
            : MHO_Operator(), fIsValid(false), fIsForward(true), fInitialized(false), fAllSpatialDimensionsAreEqual(true),
              fFillFromHostData(true), fReadOutDataToHost(true), fMaxBufferSize(0), fTotalDataSize(0), fOpenCLFlags(""),
              fFFTKernel(nullptr), fSpatialDimensionBufferCL(nullptr), fTwiddleBufferCL(nullptr),
              fConjugateTwiddleBufferCL(nullptr), fScaleBufferCL(nullptr), fCirculantBufferCL(nullptr), fDataBufferCL(nullptr),
              fPermuationArrayCL(nullptr), fWorkspaceBufferCL(nullptr), fNLocal(0)
        {
            ;
        };
        ...
        virtual void SetInput(XArgType* in) { fInput = in; };

        virtual void SetOutput(XArgType* out) { fOutput = out; };
            ...
            if(fIsValid && fInitialized)
            {
                ...
                for(unsigned int D = 0; D < XArgType::rank::value; D++)
                {
                    ...
                    if(fAllSpatialDimensionsAreEqual)
                    {
                        ...
                        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fFFTKernel, cl::NullRange, global,
                                                                                            local);
                    ...
                    else
                    {
                        ...
                        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(
                            *fTwiddleBufferCL, CL_TRUE, 0, fMaxBufferSize * sizeof(CL_TYPE2), &(fTwiddle[D][0]));
#ifdef ENFORCE_CL_FINISH
                        MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
                        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fConjugateTwiddleBufferCL, CL_TRUE,
                                                                                          0, fMaxBufferSize * sizeof(CL_TYPE2),
                                                                                          &(fConjugateTwiddle[D][0]));
#ifdef ENFORCE_CL_FINISH
                        MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
                        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(
                            *fScaleBufferCL, CL_TRUE, 0, fMaxBufferSize * sizeof(CL_TYPE2), &(fScale[D][0]));
#ifdef ENFORCE_CL_FINISH
                        MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
                        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(
                            *fCirculantBufferCL, CL_TRUE, 0, fMaxBufferSize * sizeof(CL_TYPE2), &(fCirculant[D][0]));
#ifdef ENFORCE_CL_FINISH
                        MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
                        MHO_OpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(
                            *fPermuationArrayCL, CL_TRUE, 0, fMaxBufferSize * sizeof(unsigned int), &(fPermuationArray[D][0]));
#ifdef ENFORCE_CL_FINISH
                        MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
    ...
    private:
        void ConstructWorkspace()
        {
            ...
            for(unsigned int i = 0; i < XArgType::rank::value - 1; i++)
            {
                ...
                {
                    if(MHO_FastFourierTransformUtilities< floating_point_value_type >::ComputeBluesteinArraySize(
                           fSpatialDim[i]) > fMaxBufferSize)
                    {
                        fMaxBufferSize =
                            MHO_FastFourierTransformUtilities< floating_point_value_type >::ComputeBluesteinArraySize(
                                fSpatialDim[i]);
                    }
                }
            }
            ...
            for(unsigned int i = 0; i < XArgType::rank::value - 1; i++)
            {
                ...
                MHO_FastFourierTransformUtilities< floating_point_value_type >::ComputeConjugateTwiddleFactors(
                    M, &(fConjugateTwiddle[i][0]));

                //compute the bluestein scale factors for this dimension
                MHO_FastFourierTransformUtilities< floating_point_value_type >::ComputeBluesteinScaleFactors(N,
                                                                                                             &(fScale[i][0]));
                //compute the circulant vector for this dimension
                MHO_FastFourierTransformUtilities< floating_point_value_type >::ComputeBluesteinCirculantVector(
                    N, M, &(fTwiddle[i][0]), &(fScale[i][0]), &(fCirculant[i][0]));
        ...
        void ConstructOpenCLKernels()
        {
            ...
            fFFTKernel =
                k_builder.BuildKernel(clFile.str(), std::string("MultidimensionalFastFourierTransform_Stage"), options.str());
        ...
        void BuildBuffers()
        {
            ...
            fSpatialDimensionBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
                                                       XArgType::rank::value * sizeof(unsigned int));

            //buffer for the FFT twiddle factors
            fTwiddleBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
                                              fMaxBufferSize * sizeof(CL_TYPE2));

            //buffer for the conjugate FFT twiddle factors
            fConjugateTwiddleBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
                                                       fMaxBufferSize * sizeof(CL_TYPE2));

            //buffer for the bluestein scale factors
            fScaleBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
                                            fMaxBufferSize * sizeof(CL_TYPE2));

            //buffer for the bluestein circulant vector
            fCirculantBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
                                                fMaxBufferSize * sizeof(CL_TYPE2));

            //buffer for the data to be transformed
            fDataBufferCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_WRITE,
                                           fTotalDataSize * sizeof(CL_TYPE2));

            //buffer for the permutation_array
            fPermuationArrayCL = new cl::Buffer(MHO_OpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,
        ...
        void AssignBuffers()
        {
            ...
            Q.enqueueWriteBuffer(*fSpatialDimensionBufferCL, CL_TRUE, 0, (XArgType::rank::value - 1) * sizeof(unsigned int),
                                 fSpatialDim);
#ifdef ENFORCE_CL_FINISH
            MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
            ...
            if(fAllSpatialDimensionsAreEqual)
            {
                ...
                MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
                Q.enqueueWriteBuffer(*fConjugateTwiddleBufferCL, CL_TRUE, 0, fMaxBufferSize * sizeof(CL_TYPE2),
                ...
                MHO_OpenCLInterface::GetInstance()->GetQueue().finish();
#endif
                Q.enqueueWriteBuffer(*fPermuationArrayCL, CL_TRUE, 0, fMaxBufferSize * sizeof(unsigned int),
