...
class MHO_JlScanStoreInterface
{
    public:
        ...
        MHO_JlScanStoreInterface(MHO_ScanDataStore* scan_store): fScanStore(scan_store), fIsOwned(false), fInitialized(false){};

        //! Called from the Julia side to create a stand-alone store.
        MHO_JlScanStoreInterface(): fScanStore(new MHO_ScanDataStore()), fIsOwned(true), fInitialized(false){};
        ...
        int LoadBaseline(std::string baseline)
        {
            ...
            if(fBaselineContainers.count(baseline))
            {
                return 1;
            }
        ...
        int LoadStation(std::string station)
        {
            ...
            if(fStationContainers.count(station))
            {
                return 1;
            }
            MHO_ContainerStore* con = new MHO_ContainerStore();
        ...
        int LoadFringe(std::string fringe)
        {
            ...
            if(fFringeContainers.count(fringe))
            {
                return 1;
            }
            MHO_ContainerStore* con = new MHO_ContainerStore();
        ...
        void Clear()
        {
            for(auto& kv : fBaselineContainers)
            {
                delete kv.second.second;
                delete kv.second.first;
            }
            for(auto& kv : fStationContainers)
            {
                delete kv.second.second;
                delete kv.second.first;
            }
            for(auto& kv : fFringeContainers)
            {
                delete kv.second.second;
                delete kv.second.first;
            }
    ...
    private:
        template< typename MapT > static std::vector< std::string > Keys(const MapT& m)
        {
