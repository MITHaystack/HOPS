...
template< typename XTableType > class MHO_PyTableContainer
{
    public:
        MHO_PyTableContainer(MHO_ExtensibleElement* element): fElement(element)
        {
        ...
        py::array_t< typename XTableType::value_type > GetNumpyArray()
        {
            ...
            for(std::size_t i = 0; i < fRank; i++)
            {
                strides[i] *= sizeof(typename XTableType::value_type);
            }
            py::array_t< typename XTableType::value_type > ret_val{
                fTable->GetDimensionArray(), strides, fTable->GetData(),
        ...
        py::dict GetMetaData() { return GetTableTags< XTableType >(fTable); }

        void SetMetaData(py::dict metadata) { return SetTableTags< XTableType >(fTable, metadata); }
    ...
    private:
        template< typename XDataTableType > static py::dict GetTableTags(XDataTableType* table)
        {
            py::dict tags = table->GetMetaDataAsJSON();
            return tags;
        }

        template< typename XDataTableType > static void SetTableTags(XDataTableType* table, py::dict metadata)
        {
        ...
        class PyAxisMetaDataFiller
        {
            public:
                ...
                template< typename XAxisType > void operator()(const XAxisType& axis) { *fDict = axis.GetMetaDataAsJSON(); }
        ...
        class PyAxisMetaDataSetter
        {
            public:
                ...
                template< typename XAxisType > void operator()(XAxisType& axis) { axis.SetMetaDataAsJSON(*fDict); }
        ...
        class PyAxisLabelModifier
        {
            public:
                ...
                PyAxisLabelModifier(std::size_t index, py::object* label_object): fIndex(index), fObject(label_object){};
                ~PyAxisLabelModifier(){};
                ...
                template< typename XAxisType > void operator()(XAxisType& axis)
                {
                    ...
                    {
                        msg_error("python_bindings", "error axis coordinate index out of bounds: "
...
template< typename XTableType > void DeclarePyTableContainer(py::module& m, std::string pyclass_name = "")
{
    if(pyclass_name == "")
    {
    ...
    py::class_< MHO_PyTableContainer< XTableType > >(m, pyclass_name.c_str())
        ...
        .def("get_classname", &hops::MHO_PyTableContainer< XTableType >::GetClassName, "get the name of the table class")
        .def("get_dimension", &hops::MHO_PyTableContainer< XTableType >::GetDimension,
             "get the size of the associated table dimension", py::arg("dimension_index"))
        ...
        .def("set_metadata", &hops::MHO_PyTableContainer< XTableType >::SetMetaData,
             "replace the meta data attached to this table with the passed dictionary", py::arg("metadata_object"))
        ...
        .def("get_axis", &hops::MHO_PyTableContainer< XTableType >::GetCoordinateAxis, py::return_value_policy::reference,
             "get the axis container associated with the dimension index specified", py::arg("index"))
        .def("get_axis_metadata", &hops::MHO_PyTableContainer< XTableType >::GetCoordinateAxisMetaData,
             py::return_value_policy::copy,
             "get a copy of the meta data attached to axis associated with the dimension index specified", py::arg("index"))
        .def("set_axis_metadata", &hops::MHO_PyTableContainer< XTableType >::SetCoordinateAxisMetaData,
             "replace the meta data attached to the axis associated with the dimension index specified, with passed dictionary "
             "metadata_object",
             py::arg("index"), py::arg("metadata_object"))
        .def("set_axis_label", &hops::MHO_PyTableContainer< XTableType >::SetCoordinateLabel,
             "modify the axis (specified by the dimension_index) at the coordinate location specified by coordinate_index, "
             "replacing the existing value with label_value",
             py::arg("dimension_index"), py::arg("coordinate_index"), py::arg("label_value"));
}
