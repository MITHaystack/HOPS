...
echo "Running: fourfit3 -m 4 -c ./test2.cf -b AS -P RR ./${MK4_SCAN_DIR} set ${SET_STRING} plot_data_dir ./chk_tcohere"
fourfit3 -m 4 -c ./test2.cf -b AS -P RR ./${MK4_SCAN_DIR} set ${SET_STRING} plot_data_dir ./chk_tcohere 2>&1  | tee ./ff.out

#convert the fringe file to json
hops2json ${output_file}

#use jq (json query) to extract the plot_data element and pipe to file
echo "jq '.[].tags.plot_data | select( . != null )' "${output_file}.json" > ./fdump_tcohere.json"
jq '.[].tags.plot_data | select( . != null )' "${output_file}.json" > ./fdump_tcohere.json

#now compare the results between fourfit3 and fourfit4
compjsonpdd.py -r 0.0025 ./fdump_tcohere.json ./chk_tcohere/104-1228-AS-B-RR.*
RET_VAL=$?
