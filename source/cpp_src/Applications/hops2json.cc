...
int main(int argc, char** argv)
{
    ...
    app.add_option("-s,--shortname", shortname, "specify and extract a single object by shortname (returns first matching object) ")->excludes("--uuid");
    ...
    else //we are only converting a single specified object (matching uuid or shortname)
    {
        ...
        MHO_UUID obj_uuid;
        if(shortname != ""){obj_uuid = conStore.GetObjectUUID(shortname);}
        else if(uuid_string != "")
        {
