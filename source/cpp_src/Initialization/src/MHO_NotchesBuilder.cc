...
MHO_NotchesBuilder::Build()
{
    if( IsConfigurationOk() )
    {
        ...
        if(values.size() %2 != 0)
        {
            msg_error("initialization", "cannot construct MHO_Notches, improper number ("<<values.size()<<"), of frequency limits given (multiple of 2 required)" << eom);
            return false;
        }
