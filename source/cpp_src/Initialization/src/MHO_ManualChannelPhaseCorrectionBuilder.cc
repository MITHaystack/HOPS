...
bool MHO_ManualChannelPhaseCorrectionBuilder::Build()
{
        ...
        if(chan2pcp.size() > 0)
        {
            ...
            if(vis_data == nullptr)
            {
                msg_error("initialization",
                          "cannot construct MHO_ManualChannelPhaseCorrection without visibility data." << eom);
                return false;
            }
...
std::string MHO_ManualChannelPhaseCorrectionBuilder::ParsePolFromName(const std::string& name)
{
    if(name == "pc_phases")
    {
        return std::string("?");
...
std::string MHO_ManualChannelPhaseCorrectionBuilder::ExtractStationIdentifier()
{
