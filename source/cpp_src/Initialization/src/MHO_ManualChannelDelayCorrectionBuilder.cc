...
bool MHO_ManualChannelDelayCorrectionBuilder::Build()
{
        ...
        if(chan2pcd.size() > 0)
        {
            ...
            if(vis_data == nullptr)
            {
                msg_error("initialization",
                          "cannot construct MHO_ManualChannelDelayCorrection without visibility data." << eom);
                return false;
            }
...
std::string MHO_ManualChannelDelayCorrectionBuilder::ParsePolFromName(const std::string& name)
{
    if(name == "delay_offs")
    {
        return std::string("?");
    }
    if(name == "delay_offs_x")
    {
        return std::string("X");
    }
    if(name == "delay_offs_y")
    {
        return std::string("Y");
    }
    if(name == "delay_offs_r")
    {
        return std::string("R");
    }
    if(name == "delay_offs_l")
    {
        return std::string("L");
    }
    return std::string("?");
...
std::string MHO_ManualChannelDelayCorrectionBuilder::ExtractStationIdentifier()
{
