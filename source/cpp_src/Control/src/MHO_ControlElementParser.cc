...
mho_json MHO_ControlElementParser::ParseControlStatement(const MHO_ControlStatement& control_statement)
{
    ...
    if(formatIt != fElementFormats.end())
    {
        format_loaded = true;
    }
    mho_json elem;
    ...
    {
        msg_warn("control",
                 "parser error, could not load format file for: '" << element_name << ">' element, skipping." << eom);
    }

    return elem;
}

mho_json MHO_ControlElementParser::ParseTokens(const std::string& element_name, mho_json& format,
                                               const std::vector< MHO_Token >& tokens)
{
...
mho_json MHO_ControlElementParser::ProcessCompound(const std::string& element_name, mho_json& format,
                                                   const std::vector< MHO_Token >& tokens)
{
    mho_json element_data;
    ...
    for(auto it = fields.begin(); it != fields.end(); it++)
    {
        ...
        {
            if(tokens[token_idx].fValue == nothing)
            {
                token_idx++;
            ...
            else
            {
                ...
                if(type_name == "list_int" || type_name == "list_real" || type_name == "list_string" ||
                   type_name == "fixed_length_list_string")
                {
