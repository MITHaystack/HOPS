...
void
MHO_DiFXChannelNameConstructor::AddChannelNames(mho_json& vex_root)
{
    ...
    std::string bbc_name = vex_root["$MODE"][mode_key]["$BBC"][0]["keyword"].get<std::string>(); //TODO FIXME if stations have different bbcs
    std::string if_name = vex_root["$MODE"][mode_key]["$IF"][0]["keyword"].get<std::string>(); //TODO FIXME if stations have different ifs
    ...
    for(auto it = mk4IDToFreqTableName.begin(); it != mk4IDToFreqTableName.end(); it++)
    {
        ...
        for(std::size_t i=0; i<nfreqs; i++)
        {
            ...
            for(std::size_t j=0; j<fOrderedSkyFrequencies.size(); j++)
            {
                ...
                if( std::fabs(delta) < fChanTol ){ok_to_add = false;}
            }
            if(ok_to_add){fOrderedSkyFrequencies.push_back(all_freqs[i]);}
        }
...
std::size_t
MHO_DiFXChannelNameConstructor::FindChannelIndex(double sky_freq)
{
    //brute force search
    for(std::size_t i=0; i<fOrderedSkyFrequencies.size(); i++)
    {
        double delta = sky_freq - fOrderedSkyFrequencies[i];
        if( std::fabs(delta) < fChanTol ){return i;}
    }
    return 0;
}
