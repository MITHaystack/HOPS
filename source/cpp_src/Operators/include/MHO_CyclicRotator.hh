...
/**
 * @brief Class MHO_CyclicRotator
 */
template< class XArrayType > class MHO_CyclicRotator: public MHO_UnaryOperator< XArrayType >
{
    public:
        MHO_CyclicRotator()
        {
        ...
        void SetOffset(std::size_t dimension_index, int64_t offset_value)
        {
            ...
            {
                msg_error("operators",
                          "error, rotation offset for dimension: " << dimension_index << ", exceeds array rank." << eom);
            }
            fInitialized = false;
        }
    ...
    protected:
                ...
                for(size_t d = 0; d < XArrayType::rank::value; d++)
                {
                    if(fOffsets[d] != 0)
                    {
                        ...
                        for(size_t n = 0; n < n_rot; n++)
                        {
                            ...
                            MHO_NDArrayMath::RowMajorIndexFromOffset< XArrayType::rank::value - 1 >(
                                n, non_active_dimension_size, non_active_dimension_value);
                            ...
                            index[d] = 0;
                            data_location =
                                MHO_NDArrayMath::OffsetFromRowMajorIndex< XArrayType::rank::value >(fDimensionSize, index);
        ...
        virtual bool ExecuteOutOfPlace(const XArrayType* in, XArrayType* out) override
        {
            if(fInitialized)
            {
                ...
                {
                    MHO_NDArrayMath::RowMajorIndexFromOffset< XArrayType::rank::value >(in_iter.GetOffset(), in_dim,
                                                                                        &(in_loc[0]));
                    for(std::size_t i = 0; i < XArrayType::rank::value; i++)
                    {
    ...
    private:
         ...
         * @param dim Dimension along which to rotate
         */
        template< typename XCheckType = XArrayType >
        typename std::enable_if< std::is_base_of< MHO_TableContainerBase, XCheckType >::value, void >::type
        IfTableRotateAxis(const XArrayType* in, XArrayType* out, std::size_t dim)
        ...
        {
            public:
                RotateAxis(std::size_t offset): fOffset(offset){};
                ~RotateAxis(){};
         ...
         */
        inline int64_t positive_modulo(int64_t i, int64_t n) { return (i % n + n) % n; }
