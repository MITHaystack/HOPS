...
#define FFT1_TYPE MHO_MultidimensionalFastFourierTransformFFTW< ARRAY1_TYPE >
#define FFT2_TYPE MHO_MultidimensionalFastFourierTransformFFTW< ARRAY2_TYPE >
#define FFT3_TYPE MHO_MultidimensionalFastFourierTransformFFTW< ARRAY3_TYPE >
#define FFT4_TYPE MHO_MultidimensionalFastFourierTransformFFTW< ARRAY4_TYPE >

template< typename XArrayType, typename XFFTType >
int run_test(const std::vector< std::size_t >& dim_size, const std::vector< std::size_t >& selected_axes)
{
    if(dim_size.size() != XArrayType::rank::value)
    {
        std::cout << "dimension rank error: " << dim_size.size() << " != " << XArrayType::rank::value << std::endl;
        std::exit(1);
    }
    ...
    {
        bool selected = false;
        for(std::size_t j = 0; j < selected_axes.size(); j++)
        {
            if(i == selected_axes[j])
            {
                selected = true;
            }
        }
        if(selected)
        {
            first_pass_axes.push_back(i);
        }
        else
        {
            second_pass_axes.push_back(i);
        }
    }
