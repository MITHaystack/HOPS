...
std::complex< double > MHO_FringeRotation::vrot(double time_delta, double freq, double ref_freq, double dr, double mbd) const
{
    return vrot_v1(time_delta, freq, ref_freq, dr, mbd);
}

std::complex< double > MHO_FringeRotation::vrot_v1(double time_delta, double freq, double ref_freq, double dr, double mbd) const
{
...
double MHO_FringeRotation::calc_sideband_correction(double mbd) const
{
    ...
    {
        theta_corr += oc_corr;
    } //std::cout<<"oc correction: "<<oc_corr<<std::endl; std::cout<<mbd<<", "<<fSideband<<", "<<fSBDSep<<std::endl;}
    else
    {
        theta_corr += (0.125 * fSBDMax * fSideband) / fSBDSep;
    }
    return theta_corr;
}
