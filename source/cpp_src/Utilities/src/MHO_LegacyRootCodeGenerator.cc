...
std::vector< std::string > MHO_LegacyRootCodeGenerator::GetCodes(std::size_t N)
{
...
int MHO_LegacyRootCodeGenerator::root_id_delta(time_t now)
{
    int delta = 4;
    if(now >= HOPS_ROOT_BREAK)
        delta = 1;
    return delta;
}

std::string MHO_LegacyRootCodeGenerator::root_id_later(time_t now)
{
...
std::string MHO_LegacyRootCodeGenerator::root_id(int year, int day, int hour, int min, int sec)
{
...
std::string MHO_LegacyRootCodeGenerator::root_id_break(time_t now, int year, int day, int hour, int min, int sec)
{
