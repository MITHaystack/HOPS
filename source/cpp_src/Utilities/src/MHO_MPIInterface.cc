...
void MHO_MPIInterface::PrintMessage(std::string msg)
{
    ...
    {
        MPI_Send(&(buf[msg_start_indexes[fGlobalProcessID]]), out_msg_sizes[fGlobalProcessID], MPI_CHAR, 0, MSG_TAG,
                 MPI_COMM_WORLD);
    }
...
void MHO_MPIInterface::BroadcastString(std::string& msg)
{
...
void MHO_MPIInterface::DetermineLocalRank()
{
    ...
    if(fGlobalProcessID == 0)
    {
        for(int i = 1; i < fNProcesses; i++)
        {
            MPI_Recv(&(buf[msg_start_indexes[i]]), out_msg_sizes[i], MPI_CHAR, i, HOST_DETERMINATION_TAG, MPI_COMM_WORLD,
                     &status);
        }
    }
    else
    {
        ...
        MPI_Send(&(buf[msg_start_indexes[fGlobalProcessID]]), out_msg_sizes[fGlobalProcessID], MPI_CHAR, root_rank,
                 HOST_DETERMINATION_TAG, MPI_COMM_WORLD);
    }
...
std::set< std::string > MHO_MPIInterface::MergeStringSet(const std::set< std::string >& local_set)
{
    ...
    else
    {
        ...
        MPI_Gatherv(key_lengths.data(), local_entry_count, MPI_INT, nullptr, nullptr, nullptr, MPI_INT, 0, MPI_COMM_WORLD);
