...
void
MHO_StationCodeMap::ProcessLine()
{
    if(fLine.size() != 0)
    {
        ...
        if( TokensAreValid() )
        {
            ...
            if(fMk4Ids.count(mk4id) != 0 ){ok = false;}
...
MHO_StationCodeMap::TokensAreValid()
{
    ...
    if(fTokens[0].length() != 1){return false;}
    if(fTokens[1].length() != 2){return false;}
    return true;
}
...
std::string
MHO_StationCodeMap::GetMk4IdFromStationCode(std::string station_code)
{
    std::string mk4id = "";
    if(station_code.length() != 2){return mk4id;}
...
MHO_StationCodeMap::ToUpperCase(std::string token)
{
    ...
    {
        if( isalpha(token[i]) ){ value[i] = toupper(token[i]);}
        else { value[i] = token[i];}
    }
    return value;
}
...
MHO_StationCodeMap::ToLowerCase(std::string token)
{
    ...
    for(std::size_t i=0; i<token.size(); i++)
    {
        if( isalpha(token[i]) ){ value[i] = tolower(token[i]);}
        else{ value[i] = token[i];}
    }
    return value;
}
...
MHO_StationCodeMap::ToCanonicalCase(std::string token)
{
    ...
    if( isalpha(token[0]) ){ value[0] = toupper(token[0]);}
    else{ value[0] = token[0];}
    //rest are lower case
