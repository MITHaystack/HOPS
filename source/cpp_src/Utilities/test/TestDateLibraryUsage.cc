...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    std::cout << "hops epoch nanoseconds from system utc epoch start = " << hops_epoch_start_utc.time_since_epoch().count()
              << std::endl;
    std::cout << "hops epoch nanoseconds from system tai epoch start = " << hops_epoch_start_tai.time_since_epoch().count()
              << std::endl;
    ...
    std::cout << "to hops date in iso-8601 format: " << hops_clock::to_iso8601_format(hops_clock::from_vex_format(vtest))
              << std::endl;
    ...
    std::cout << "number of leap seconds in tzdb: " << get_tzdb().leap_seconds.size() << " -> should be 27 (as of 9/12/2023)"
              << std::endl;
