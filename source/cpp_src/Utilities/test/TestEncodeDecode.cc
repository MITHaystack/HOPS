...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    std::vector< uint64_t > values = {0,  1,  10,  15,  16,  17,  25,  26,  27,  31,  32,   33,   63,   64,   65,   51,  52,
                                      53, 99, 100, 101, 255, 256, 257, 675, 676, 677, 2703, 2704, 2705, 4095, 4096, 4097};
    ...
    for(std::size_t b = 0; b < bases.size(); b++)
    {
        for(std::size_t i = 0; i < values.size(); i++)
        {
            ...
            if(delta != 0)
            {
                std::cout << "encode/decode error." << std::endl;
                return 1;
            }

            std::cout << "base: " << bases[b].size() << " (value, code, decode) = (" << values[i] << "," << enc_val << ","
                      << dec_val << ")" << std::endl;
        }
    }
