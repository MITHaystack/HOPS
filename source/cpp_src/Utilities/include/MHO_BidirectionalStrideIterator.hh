...
template< typename XValueType > class MHO_BidirectionalStrideIterator
{
    public:
        ...
        MHO_BidirectionalStrideIterator(pointer begin_ptr, pointer ptr, std::size_t length, std::size_t stride)
            : fBegin(begin_ptr), fPtr(ptr), fLength(length), fStride(stride){};

        MHO_BidirectionalStrideIterator(const self_type& copy)
            : fBegin(copy.fBegin), fPtr(copy.fPtr), fLength(copy.fLength), fStride(copy.fStride){};
        ...
        std::ptrdiff_t operator-(const self_type& iter) { return std::distance(iter.GetPtr(), fPtr); }
