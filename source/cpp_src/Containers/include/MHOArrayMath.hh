#ifndef MHOArrayMath_HH__
#define MHOArrayMath_HH__
    ...
    public:
        ...
        RowMajorIndexFromOffset(std::size_t offset, const std::size_t* DimSize, std::size_t* Index)
        {
            ...
            for(std::size_t d=0; d < RANK; d++)
            {
                ...
                if(d == 0)
                {
                    Index[i] = MHOArrayMath::Modulus(offset, DimSize[i]);
                    div[i] = (offset - Index[i])/DimSize[i];
                }
                else
                {
                    Index[i] = MHOArrayMath::Modulus(div[i+1], DimSize[i]);
        ...
        {
            std::size_t total_array_size = MHOArrayMath::TotalArraySize<RANK>(DimSize);
            std::size_t index[RANK];
            for(std::size_t i=0; i<total_array_size; i++)
            {
                MHOArrayMath::RowMajorIndexFromOffset<RANK>(i, DimSize, index);
                for(std::size_t j=0; j<RANK; j++){ index[j] = (DimSize[j] - index[j])%DimSize[j];};
                ReversedIndex[i] = MHOArrayMath::OffsetFromRowMajorIndex<RANK>(DimSize, index);
            }
        }
...
template <>
struct MHOArrayMath::PowerOfTwo<0>
{
    enum { value = 1 };
};
...
struct MHOArrayMath::Divide<numerator, 1>
{
