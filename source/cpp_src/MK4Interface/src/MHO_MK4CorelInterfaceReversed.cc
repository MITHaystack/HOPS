...
MHO_MK4CorelInterfaceReversed::MHO_MK4CorelInterfaceReversed()
    : fVisibilityData(nullptr), fWeightData(nullptr), fGeneratedCorel(nullptr), fOutputDir(""), fOutputFile(""),
      fRootFilename(""), fRootFileBasename("")
{
    fNPPs = 0;
    fNAPs = 0;
    fNChannels = 0;
...
void MHO_MK4CorelInterfaceReversed::FreeAllocated()
{
...
void MHO_MK4CorelInterfaceReversed::SetOutputDirectory(const std::string& output_dir)
{
    fOutputDir = MHO_DirectoryInterface::GetDirectoryFullPath(output_dir);
}

void MHO_MK4CorelInterfaceReversed::GenerateCorelStructure()
{
    if(fVisibilityData && fWeightData)
    {
        ...
        msg_debug("mk4interface", "visibility data dimensions are: " << fNPPs << " pol products, " << fNAPs << " APs, "
                                                                     << fNChannels << " channels, " << fNSpectral
                                                                     << " spectral points per channel" << eom);

        if(fNChannels > MAXFREQ)
        {
            msg_warn("mk4interface", "the visibility data has: "
                                         << fNChannels << " channels, but the mark4 dfio library does not support more than "
                                         << MAXFREQ << " channels." << eom);
        }
        ...
        else
        {
            msg_error("mk4interface",
                      "failed to construct output file name, baseline_shortname and root_code tags missing from visibility data"
                          << eom);
            return;
        }
    }
...
{
    if(!fGeneratedCorel || !fGeneratedCorel->index)
        return;
    ...
    for(std::size_t pp = 0; pp < fNPPs; pp++)
    {
        ...
        if(pprod.size() == 2)
        {
            ...
            if(count == 0)
            {
                CreateType101Record(0, 0, ref_pol, rem_pol);
                count++;
            }
...
void MHO_MK4CorelInterfaceReversed::CreateType101Record(int count, std::size_t ch, char ref_pol, char rem_pol)
{
    ...
    msg_debug("mk4interface",
              "Generated type_101 record " << count << " for channels " << ref_chan_id << ":" << rem_chan_id << eom);
}
...
int MHO_MK4CorelInterfaceReversed::WriteCorelFile()
{
    ...
    if(retval <= 0)
    {
        msg_error("mk4interface", "failed to write corel file: " << outfile << ", error code: " << retval << eom);
    }
    else
    {
