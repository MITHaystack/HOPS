    ...
    for(auto field: fBlockFormat["fields"].items())
    {
        ...
        if(element.contains(field_name))
        {
            ...
            if(par_type.find("list_compound") != std::string::npos)
            {
                ...
                {
                    std::string line = fPad + fLineGen.ConstructElementLine(field_name, element[field_name][j], fBlockFormat["parameters"][field_name]);
    ...
    for(auto field: fBlockFormat["fields"].items())
    {
        ...
        if(element.contains(field_name))
        {
            ...
            if(par_type.find("reference") != std::string::npos)
            {
                for(std::size_t j=0; j<element[field_name].size(); j++)
                {
                    std::string line = fPad + MHO_VexDefinitions::RefTag() + fSpace + field_name + fSpace + MHO_VexDefinitions::AssignmentOp() + fSpace;
