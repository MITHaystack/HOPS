...
mho_json MHO_FringePlotInfo::construct_plot_data(MHO_ContainerStore* conStore, MHO_ParameterStore* paramStore,
                                                 MHO_OperatorToolbox* toolbox, mho_json& vexInfo)
{
    ...
    {
        mk_plotdata.EnableOptimizeClosure();
    } //this does have an effect on overall fringe phase
    if(is_mbd_anchor_set)
    {
        mk_plotdata.SetMBDAnchor(mbd_anchor);
    }
...
void MHO_FringePlotInfo::fill_plot_data(MHO_ParameterStore* paramStore, mho_json& plot_dict)
{
    ...
    if(do_ion)
    {
        win = paramStore->GetAs< std::vector< double > >("/fringe/ion_win");
    }
    else
    {
        win[0] = 0.0;
