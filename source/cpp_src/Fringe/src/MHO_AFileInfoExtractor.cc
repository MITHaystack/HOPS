...
{
    for(std::size_t i=0; i<CHAR_BUF_SIZE; i++){buf[i] = '\0';}
}
...
*/
std::string
afile_header(int version, int type, char afile_com_char)
{
...
std::string 
MHO_AFileInfoExtractor::GetAlistHeader(int version, int type, char comment_char)
{
...
std::string
MHO_AFileInfoExtractor::ConvertToAlistRow(const mho_json& data, int version)
{
