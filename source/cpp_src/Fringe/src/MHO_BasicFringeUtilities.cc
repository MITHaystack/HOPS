...
void MHO_BasicFringeUtilities::calculate_fringe_solution_info(MHO_ContainerStore* conStore, MHO_ParameterStore* paramStore,
                                                              const mho_json& vexInfo)
{
    ...
    std::string year_doy = MHO_BasicFringeInfo::leftpadzeros_integer(4, frt_ldate.year) + ":" +
    ...
    if(mbd_anchor == "sbd")
    {
        ...
        MHO_BasicFringeInfo::correct_phases_mbd_anchor_sbd(ref_freq, freq0, freq_spacing, delta_mbd, tot_phase_deg,
                                                           resid_phase_deg);
    }
...
double MHO_BasicFringeUtilities::calculate_residual_phase(MHO_ContainerStore* conStore, MHO_ParameterStore* paramStore)
{
    double total_summed_weights = paramStore->GetAs< double >("/fringe/total_summed_weights");
    double ref_freq = paramStore->GetAs< double >("/control/config/ref_freq");
...
void MHO_BasicFringeUtilities::determine_sample_rate(MHO_ContainerStore* conStore, MHO_ParameterStore* paramStore)
{
    ...
    if(vis_data != nullptr)
    {
        ...
        {
            msg_warn("fringe",
                     "multiple channel bandwidths detected when determining sample rate, SNR may not be accurate" << eom);
        }
...
double MHO_BasicFringeUtilities::calculate_snr_correction_factor(MHO_ContainerStore* conStore, MHO_ParameterStore* paramStore)
{
    ...
    if(net_ap > 0)
    {
        bw_corr = std::sqrt(net_bw / net_ap);
    }
...
void MHO_BasicFringeUtilities::calculate_ion_covariance(MHO_ContainerStore* conStore, MHO_ParameterStore* paramStore)
{
    double total_summed_weights = paramStore->GetAs< double >("/fringe/total_summed_weights");
    double ref_freq = paramStore->GetAs< double >("/control/config/ref_freq");
    double mbd = paramStore->GetAs< double >("/fringe/mbdelay");
    ...
    std::complex< double > sum_all = 0.0;
    std::vector< std::complex< double > > chan_phasors;
    chan_phasors.resize(nchan, 0);
    std::vector< double > chan_freqs;
    chan_freqs.resize(nchan, 0);
    ...
    for(std::size_t ch = 0; ch < nchan; ch++)
    {
        ...
        {
            c = 1.0 / sumwt;
        } //TODO replace 1.0 with amp_corr_fact
        chan_phasors[ch] *= c;
    }
