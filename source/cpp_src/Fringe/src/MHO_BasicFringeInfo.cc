...
std::string MHO_BasicFringeInfo::leftpadzeros_integer(unsigned int n_places, int value)
{
    std::stringstream ss;
...
std::string MHO_BasicFringeInfo::make_legacy_datetime_format(legacy_hops_date ldate)
{
...
std::string MHO_BasicFringeInfo::make_legacy_datetime_format_v2(legacy_hops_date ldate)
{
    //formats the time as:
    //YYYY:DDD:HHMMSS
    ...
    int isec = (int)ldate.second;
    std::string dt;
    dt = leftpadzeros_integer(4, iyear) + ":" + leftpadzeros_integer(3, iday) + ":" + leftpadzeros_integer(2, ldate.hour) +
         leftpadzeros_integer(2, ldate.minute) + leftpadzeros_integer(2, isec);
    return dt;
}

double MHO_BasicFringeInfo::calculate_snr(double effective_npol, double ap_period, double samp_period, double total_ap_frac,
...
double MHO_BasicFringeInfo::calculate_mbd_no_ion_error(double freq_spread, double snr)
{
    double mbd_err = (1.0 / (2.0 * M_PI * freq_spread * snr));
    return mbd_err;
}

double MHO_BasicFringeInfo::calculate_sbd_error(double sbd_sep, double snr, double sbavg)
{
...
double MHO_BasicFringeInfo::calculate_drate_error_v1(double snr, double ref_freq, double total_nap, double ap_delta)
{
...
double MHO_BasicFringeInfo::calculate_drate_error_v2(double snr, double ref_freq, double integration_time)
{
...
double MHO_BasicFringeInfo::calculate_pfd(double snr, double pts_searched)
{
...
double MHO_BasicFringeInfo::calculate_phase_error(double sbavg, double snr)
{
    //no ionosphere
...
double MHO_BasicFringeInfo::calculate_phase_delay_error(double sbavg, double snr, double ref_freq)
{
    //no ionosphere
...
double MHO_BasicFringeInfo::calculate_theory_timerms_phase(double nseg, double snr)
{
...
double MHO_BasicFringeInfo::calculate_theory_timerms_amp(double nseg, double snr)
{
...
double MHO_BasicFringeInfo::calculate_theory_freqrms_phase(double nchan, double snr)
{
    double th_freqrms_phase = std::sqrt(nchan) * 180. / (M_PI * snr);
    return th_freqrms_phase;
}

double MHO_BasicFringeInfo::calculate_theory_freqrms_amp(double nchan, double snr)
{
...
void MHO_BasicFringeInfo::correct_phases_mbd_anchor_sbd(double ref_freq, double freq0, double frequency_spacing,
                                                        double delta_mbd, double& totphase_deg, double& resphase_deg)
{
    //see fill_208.c
...
void MHO_BasicFringeInfo::ion_covariance(int nfreq, double famp, double snr, double ref_freq,
                                         const std::vector< double >& chan_freqs,
                                         const std::vector< std::complex< double > >& chan_phasors,
                                         std::vector< double >& ion_sigmas)
{
    int i, j, fr;
    double sigma_fr, fk, f0, w,
    ...
    msg_debug("fringe", "ionospheric sigmas: delay " << 1e3 * ion_sigmas[0] << " (ps), phase " << 360.0 * ion_sigmas[1]
                                                     << " (deg), dTEC " << ion_sigmas[2] << eom);
