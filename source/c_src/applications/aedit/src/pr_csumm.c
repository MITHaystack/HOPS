...
int pr_csumm(void)
{
        ...
        for(i=0; i<150; i++){buf[0] = '\0'; buf2[0] = '\0';}
        for(i=0;i<19;i++) {
            sprintf(buf,"%s%c ",buf2,qclist[i]);
            if(csumm.qcodes[i] >= 10000) sprintf(buf,"%s    ",buf2);
            else if(csumm.qcodes[i] >= 1000) sprintf(buf,"%s   ",buf2);
            else if(csumm.qcodes[i] >= 100) sprintf(buf,"%s  ",buf2);
            else if(csumm.qcodes[i] >= 10) sprintf(buf,"%s ",buf2);
            sprintf(buf2, "%s", buf);
        }
        printf("\t%s\n",buf2);
        printf("\t");
