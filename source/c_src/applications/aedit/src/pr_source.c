...
int pr_source(srcsum *src)
    ...
    for(i=0;i<19;i++) {
        ...
        if(src->qcodes[i] >= 10000) sprintf(buf,"%s    ",buf2);
        else if(src->qcodes[i] >= 1000) sprintf(buf,"%s   ",buf2);
        else if(src->qcodes[i] >= 100) sprintf(buf,"%s  ",buf2);
        else if(src->qcodes[i] >= 10) sprintf(buf,"%s ",buf2);
        sprintf(buf2, "%s", buf);
    }
    printf("\t%s\n",buf2);
    printf("\t");
