...
The HOPS embedded Python interpreter searches for plugins in two directories:
...
To use a custom user directory, export before running fourfit:
...
giving read/write access to visibilities, weights, parameters, and scan data.

2.1 Required API
-----------------

Your Python module must define a function with this exact signature:
...
The function takes a single argument: an ``MHO_PyFringeDataInterface``.
It returns nothing (``None``). Any exception raised will be caught by the
C++ side, logged, and execution will attempt to continue.
...
Add one line per operator in your ``.cf`` control file:
...
.. code-block:: python
   ...
   def correct_phase_jump(fringe_data_interface):
       """Apply a phasor correction to visibility data."""
       ...
       time_axis    = vis_obj.get_axis(2)     # time integrations
...
You MUST import ``pyMHO_Calibration`` for pybind11 to downcast ``MHO_Operator*``
...
Instead of writing a DSL control file (``.cf``), you can write a Python script
(``.py``) that defines the same configuration programmatically.
...
``Config`` accumulates control statements. Every known control keyword is
...
File: ``my_config.py`` (used as control file directly)
...
To use this as a control file, pass it to fourfit just like a ``.cf`` file:

.. code-block:: bash

   fourfit -c my_config.py ...
...
- Axis 0: polarization products (e.g. "RR", "LL", "RL", "LR")
