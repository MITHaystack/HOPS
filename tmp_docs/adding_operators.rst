...
to the MHO_ParameterStore for downstream consumption, and (2) a new calibration
operator applied to visibility data.
...
The HOPS data processing pipeline is driven by a control file (text) whose
keywords are described by JSON format descriptors. The pipeline has four stages:

.. code-block:: text

   Control File    -->   ParameterStore    -->   OperatorBuilders    -->   Operators
   (text keywords)   (path-based JSON)      (construct + configure)   (Initialize/Execute)

Key modules:

- **Control/** : Parses control files, evaluates conditionals, tokenizes
  arguments into structured JSON using format descriptors.
- **Initialization/** : Contains MHO_OperatorBuilder subclasses that construct
  operator instances, configure them with data/parameters,
  and register them with the MHO_OperatorToolbox.
- **Calibration/** : Contains the actual operator implementations (the code
  that transforms visibility data).
- **Operators/** : Base class hierarchy (MHO_Operator, MHO_UnaryOperator<>,
  MHO_BinaryOperator<>, MHO_InspectingOperator<>).
- **Utilities/** : MHO_ParameterStore, MHO_ContainerStore (data containers).

Format descriptors live in:
...
Step-by-step flow for a single scan/baseline:

1. **MHO_ControlFileParser::ParseControl()**

   - Reads the raw control file, strips comments, tokenizes
...
2. **MHO_ControlElementParser::ParseControlStatement()**
   ...
   - Produces a structured JSON value for each statement

3. **MHO_ControlConditionEvaluator::GetApplicableStatements()**

   - Evaluates "if station X and source Y" conditionals per scan
...
4. **MHO_ParameterManager::ConfigureAll()**
   ...
   - Parameter statements are removed from further processing
...
5. **MHO_OperatorBuilderManager::BuildOperatorCategory(category)**
   ...
   - Iterates applicable control statements matching the category
...
6. **Each MHO_OperatorBuilder::Build()**
   ...
   - Validates configuration (IsConfigurationOk)
   - Extracts parameters from fAttributes and/or fParameterStore
   - Retrieves data arrays from fContainerStore
   - Constructs the operator, calls SetArgs(), configures it
   - Registers with ``fOperatorToolbox->AddOperator(op, name, category)``

7. **MHO_FringeFitter executes operators by category/priority**

   - Calls ``operator->Initialize()`` then ``operator->Execute()``
   - Operators modify visibility/weight data in-place or out-of-place
...
Example: ``ref_freq 1400.0`` stores 1400.0 at ``/control/config/ref_freq``
...
No C++ code changes are needed for simple parameters. The
MHO_ParameterConfigurator handles all supported types generically.
...
condition token. Multiple condition blocks produce multiple paths.

STEP 3.3 — Consuming the parameter downstream
----------------------------------------------

In a builder or operator, retrieve the parameter:
...
Simply placing the file in ``format/control/`` or ``format/control_extensions/`` is
sufficient. Files are installed to ``DATA_INSTALL_DIR/control/`` at build time.
...
the builder must be a C++ class inheriting from MHO_OperatorBuilder.
...
.. list-table::
   ...
   * - ``MHO_InspectingOperator<T>``
     - Read-only inspection of a single const array. Produces results in internal
       workspace or parameter store.
   ...
   * - ``MHO_Operator`` (direct)
     - Rare. No template helpers. You manage all arguments via custom setters.
       Must implement: ``Initialize()`` and ``Execute()``

Common data types (from ``MHO_ContainerDefinitions.hh``):

- **visibility_type** : 4D complex [polprod, channel, time, freq]
- **weight_type** : 4D real [polprod, channel, time, freq]
- **sbd_type** : 4D complex (single-band-delay workspace)
- **station_coord_type** : station coordinate data

For 90% of calibration operators, inherit ``MHO_UnaryOperator<visibility_type>``.
...
.. note::
   ...
   Always provide both constructors. The second is for unit testing.
...
Builder accessible members from MHO_OperatorBuilder base class:

- **fOperatorToolbox** : ``MHO_OperatorToolbox*`` (register operators here)
...
For simple types (int, real, string, bool, list_\*, fixed_length_list_string):
...
For compound types (multiple fields):
...
- ``"fields"`` lists the expected tokens in order from the control file
...
``CreateNullFormatBuilders()`` with an inline format:
...
Also add the ``#include`` for the builder header at the top of the file.


5. JSON Format Descriptor Reference
====================================

Full specification of JSON format descriptor fields:
...
file in ``format/control/`` or ``format/control_extensions/``.
...
.. code-block:: text
   ...
   [ ] 3. No C++ code changes, no CMake changes needed
...
Control file::
   ...
   if station G mount_type cosecant_mount
...
Stored at: ``/control/station/Ed/mount_type``, ``/control/station/Gf/mount_type``
...
- ``ExecuteInPlace()`` iterates channels, zeroes DC spectral point
...
Control file: ``pc_phases chan1,chan2 10.5 20.3``
...
Builder reads all configuration from ParameterStore and ContainerStore.
...
5.  Optional fields must be prefixed with ``"!"`` in the "fields" array
6.  Builder must always provide both constructors (production + test)
7.  Operators registered with ``replace_duplicates=true`` will be re-added per pass
    (useful for per-station corrections); ``false`` means the first registration wins
...
10. Station-specific operators should use ``GetMatchingStationIdentifiers()`` to
    filter to the current baseline's stations
