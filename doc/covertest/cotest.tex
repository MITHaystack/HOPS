...
The tests described in this document are grouped into the following categories \FIX[NOTE, these are draft descriptions]:
...
\item[] \textbf{Oracle tests} are direct comparisons between HOPS3 and HOPS4. They are implemented using an ``oracle'', a software package that runs HOPS3 and HOPS4 functions on the same dataset. \FIX[Is this right? How are these different from regression tests?]
\item[] \textbf{Component tests} are tests of packages and dependencies. 
