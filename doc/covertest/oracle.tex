...
\reqid{A} HOPS4 shall maintain the \acs{fourmer} tool, which shall properly
  relabel channels when it combines two sub-bands.
    \reqex{\texttt{chk\_fourmer.sh} currently tests fourmer.}
\reqid{A} The \acs{fringex} program shall retain existing functionality to rotate
  ...
  \reqex{Currently, \texttt{chk\_fringex.sh} and \texttt{chk\_fsumm.sh} performs such a test with captured \acs{A-list} data.}
\reqid{A} The ``average'' capability, implemented by \acs{average} used in
  ...
  \reqex{Currently, \texttt{chk\_fsumm.sh} performs such a test with captured \acs{A-list} data. 
    The \texttt{chk\_average.sh} program tests for the existing \acs{average} application, however 
    this test needs refactoring in C or C++ for better efficiency.}
...
\reqid{P} It should be possible to make 2D plots of the type currently possible
  within \texttt{\acs{aedit}} of HOPS3.
    \reqex{This is tested with \texttt{chk\_aedit.sh}.}
