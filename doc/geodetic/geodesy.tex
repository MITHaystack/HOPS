...
\title{Geodetic Considerations in a new HOPS}
...
\abstract{%
\large
The \ac{ngEHT} project has provided funds so that \ac{HOPS} may
be modernized and improved to support the needs of a \ac{ngEHT}.
We are taking this opportunity to re-work the code so that no
existing functionality is lost, and at the same time, improving
it so that it becomes easier to maintain for the future.  This
document covers some things that are not part of the formal
\ac{ngEHT} project which provides the team with guidlines for
development as may be funded separately by the geodetic programs
at Haystack.
}
