...
\begin{algorithm}[h!]
  \caption{Complex conjugation operator.}
    \begin{algorithmic}[1]
    \Require{Complex array $\mathbf{X}$ of rank $N$, and total size $n$.}
	\State {\textbf{for} $j=0\ldots n$ \textbf{do} }
...
\begin{algorithm}[h!]
  \caption{Cyclic rotation operator.}
    \begin{algorithmic}[1]
    \Require{Array $\mathbf{X}$ of rank $N$, with dimensions of $\{n_1, n_2, \ldots, n_{N-1} \}$ and total size $M$. The integer offsets of each axis to be rotated $m_i$.}
	\State {\textbf{for} $j=0\ldots n $ \textbf{do} }
