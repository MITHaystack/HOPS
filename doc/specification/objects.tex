...
In addition to the raw data stored in the N-dimensional array, it is important to associate a coordinate axis with each dimension. For
example, in the case of channelized visibilities, the axes of each of the four dimensions would be:
...
It should be said that these coordinate axes are there merely to label the data, they are not meant to provide a reverse-look-up
capability, for example inverting the polarization-product code ``XX'' to infer the 0-th index location of 0. Array access is still
to be done using unsigned integer values.
