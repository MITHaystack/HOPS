...
document will be updated periodically along with the software as it is built out with an initial focus on components of 
the design which must be in place in order for further development to progress.
...
a fringe plot utility for diagnostic information about the quality of an individual scan-baseline fringe, as well as the equivalent executables to \textit{alist} and \textit{aedit}, which summarize multiple fringe solutions and display condensed information about multiple scans respectively.
...
The build system for this project will be the automake\urlfootnote{https://www.gnu.org/software/automake/} or CMake build system\urlfootnote{https://cmake.org}, and version control will proceed through a locally hosted (Haystack) git repository. HOPS3 used the automake build system and there is some
...
both build systems may be implemented to see which provides an optimal choice. Version control will be handled in a local \textit{git} repository during
the prime development phase. Eventually a public \textit{git} repository will be made available for releases which will also make it much easier to leverage community contributions if so desired at some point in the future.
...
\begin{enumerate}
 \item Data Containers: These serve to organize large n-dimensional arrays of a single data type (e.g. visibility data and correction/calibration table data). 
...
\begin{enumerate}
 ...
 \item TableContainer - encapsulates rank-N tensor-like data with associated item and axis (vector) units
...
pybind11 or boost.Python & Required & C/C++ $\leftrightarrow$ Python interface and bindings & None \\ \hline
...
\begin{enumerate}
 \item We have to refactor our dependency management for libraries to be more robust; The dependency management has to take in to account in 
 Geof's words smart and stupid linkers where the order of libraries being imported may matter. See f1d65cb6e7aa87eb40a6f2c1e589770d1490819d.
