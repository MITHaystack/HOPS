...
* HOPS4 installed (with plotting enabled via gnuplot or python-matplotlib) in <hops-install> and the environment sourced:

  .. code-block:: bash

     export HOPS_INSTALL_DIR=<hops-install>
     source $HOPS_INSTALL_DIR/bin/hops.bash
...
* The ``vt9105`` test dataset downloaded. To obtain the test data archives run:
  ...
  This will place the data under ``$HOPS_INSTALL_DIR/data/test_data/``.
  You can then unpack it with:

  .. code-block:: bash

     cd $HOPS_INSTALL_DIR/data/test_data/
  ...
  .. code-block:: bash
     ...
     DATADIR=$HOPS_INSTALL_DIR/data/test_data/vt9105
     cd $DATADIR
