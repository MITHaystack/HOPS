...
.. code:: cpp
   ...
   class MHO_Operator
   {
       public:
           ...
           //name and priority (priority determines execution order within a category)
...
.. code:: c++
   ...
   //(MHO_ScalarMultiply is illustrative here; a similar effect can be achieved
   //via MHO_FunctorBroadcaster with a scalar-multiply functor)
   MHO_ScalarMultiply mult;
   mult.SetFactor(2);
   mult.SetArgs(array1);

   MHO_SubSample sub;
   sub.SetDimensionAndStride(0, 4);
...
some information from it (e.g. finding a maximum in an array). A fourth base class,
:hops:`MHO_UnaryInPlaceOperator`, exists for unary operators that only support
in-place operation (no out-of-place variant).
...
Operator Type              Functor (used via MHO_FunctorBroadcaster)
...
``std::conj()`` to each element. This is a functor class (derived from
:hops:`MHO_UnaryFunctor`, not from :hops:`MHO_Operator`) and must be wrapped by
:hops:`MHO_FunctorBroadcaster` to be used as a pipeline operator; it supports both
in-place and out-of-place operation via that wrapper.
...
Applies the absolute value operation to every element of the input array. This is a
functor class (derived from :hops:`MHO_UnaryFunctor`, not from :hops:`MHO_Operator`)
and must be wrapped by :hops:`MHO_FunctorBroadcaster` to be used as a pipeline
operator; it supports both in-place and out-of-place operation via that wrapper.
...
Replaces NaN values in the input array with a specified mask value. This is a
functor class (derived from :hops:`MHO_UnaryFunctor`, not from :hops:`MHO_Operator`)
and must be wrapped by :hops:`MHO_FunctorBroadcaster` to be used as a pipeline
operator; it supports both in-place and out-of-place operation via that wrapper.
...
more useful manipulations of the data (e.g. ``MHO_NormFX``). The
...
complicated process (as opposed to the all-or-nothing approach of the
(HOPS3) function norm_fx.c).
