...
the display mechanics of a figure). \FIX[Should a documented report
be generated?]
...
\FIX[Warning: the enumeration of each requirement is going to change with
...
affected. \reqex{For example FFT calls should be wrapped so that other
...
\reqid{A} \ac{MHO} shall be support input from the DiFX correlator (in Swinburne format).
...
\reqid{A} \ac{MHO} shall support input from \acs{VEX} version 1.5 \footnote{\acs{VEX} 1.5
...
\reqid{D} \ac{MHO} shall provide an application to support export of native
data types to a TBD archival format (e.g HDF5).

\reqid{D} A c-library to interface with the legacy \acs{Mk4} data types
...
\FIXME[Re-write these as specific requirements, move check script details to the
...
\reqid{A} The ``average'' capability, implemented by \acs{average}
      ...
      and should be re-implemented in a more easily used (and likely more
...
\reqid{P} Implement data visualization tools in Python for \acs{fourfit}
\reqex{the \acs{HOPS} version of \acs{fourfit} provides a fixed fringe summary
plot. \acs{MHO} shall have an interactive plotting capability to enable zooming
...
\reqid{A} The \ac{MHO} suite must provide mechanisms to preserve the correlator 
output data.
  \reqex{Disk space is cheaper these days; a \acs{PERL} script,
    \texttt{hops\_data\_links.pl} exists to manage symbolic links to
    analysis files in a working directory (separate) from the original
...
\reqid{A} It shall be possible to automatically discard correlator
    Acquisition Periods with small weights.
  \reqex{When data are poorly recorded, the correlation product is the
    ...
    capability. In addition, user supplied ad-hoc flagging capability
   is tested by \texttt{chk\_flagging.sh}.}

\reqid{A} \ac{MHO} shall have the ability to flag data based on a user supplied list
...
\reqid{A} Implement an algorithm for solving for station-based quantities
  ...
  \reqex{This is new for \acs{MHO}. \FIX[Need some additional details here - 
  how is the test for this requirement going to be implemented?]}
