...
version 6.
\reqex[Rationale]{\acs{A-list} files are an ASCII tab-separated column file which serve as a summary format containing the results of the \texttt{fourfit} post-processing for each scan/baseline.  Compatibility with the existing version is necessary for backwards compatibility and regression testing.}
...
preserved.
\reqex[Rationale]{The existing control file should continue to be supported. Any new
...
\reqex[Rationale]{When data is missing the correlation products will be noisy, possibly to incorrect results. Flagging poorly recorded data segments is currently done in HOPS3 provided \acs{DiFX} properly notices the loss and assigns a low weight to the \acs{AP} bin. The threshold value below which low-weight data is discarded is user determined.}
%The HOPS3 script
