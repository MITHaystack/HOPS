...
the display mechanics of a figure). These tests shall have explicit instructions
for the user and the results shall be recorded.

\item[D], for desires. These are general design goals, which are not
esential requirements for HOPS4 and are not directly testable,
...
\reqid{A} HOPS4 shall support at least 64 channels but should not impose a
...
\reqex{The \acs{PGPLOT} package has long been obsolete, and any replacement
...
\reqid{A} HOPS4 shall be support input from the DiFX correlator (in Swinburne format).
...
in Python in HOPS4 and shall support alist file version 5 \& 6 \footnote{If
there is an \acs{A-list} version 7, these shall apply to version 7 as well. A discussion
of a TDB version 7 \acs{alist} shall be included in the specifications document.}.
...
HOPS3 package.  ``complex'' refers to both amplitude and phase variation 
...
capability.  The desire is for a more flexible arrangment that is not limited by 
channel boundaries.}
...
need to be preserved, although we shall most likley do so.}


\reqid{D} HOPS4 shall preserve the the Single Band Delay, Multi Band Delay 
and Delay Rate search algorithm in its current form. However, in addition it will 
be more modular such that the user may (de)select certain search dimensions.
\reqex{The current loop in \acs{fourfit} includes a loop over \acs{TEC} as well, 
which makes 4 loops.  For spectral line data at high frequencies the ionosphere 
is unimportant and where \acs{MBD} impossible, this is two loops too many}.
