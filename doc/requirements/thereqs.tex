...
\reqid{A} \ac{MHO} should support up to
  ...
  \reqex{\ac{MHO} requires upper bounds merely to verify that
    it supports ``more than enough''}
\reqid{A} the number of \acsp{channel} may exceed 64 and should
    have no practical limit.
  \reqex{\ac{MHO} requires an upper bound merely to implement
    with some sensible \ac{channel} labelling scheme}%
...
\begin{description}
% ./vex2xml/Makefile.am
\reqid{A} every program should provide useful
...
\reqid{A} it should be possible to automatically discard correlator
  ...
  \reqex{When data is poorly recorded, the correlation product is the
    ...
    The ability to \ac{flag} such input should not be arduous.  The
...
\reqid{A} Provide library access to \acs{LSF} methods
...
\reqid{D} Implement data visualization tools in Python for \acs{fourfit}
  \reqex{the \acs{HOPS} version of \acs{fourfit} provides a fixed fringe summary
    plot.  Often one notes some issue requiring some examination, or at
