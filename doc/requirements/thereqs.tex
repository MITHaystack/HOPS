...
\reqid{A} Every code fragment shall have a \texttt{test\_} module, and unit 
tests shall cover all code.
\reqex{This is a somewhat circular - does a requirement for tests belong in the 
...
\begin{description}
% ./vex2xml/Makefile.am
\reqid{A} every function shall provide useful
...
\acs{VEX} to \acs{XML}, for \acs{VEX} version 1.5.
    \reqex{\texttt{chk\_v2v.sh} performs this test against a dozen separate
...
compression used in the \acs{Mk4} fringe file format. \FIX[This requirement will likely 
...
\reqid{D} The \acs{Mk4} data types are stored big-endian; the new types
...
%    a bigendian processor in the future.}

%% DH note: The Mk4 data type is the output from the difx2mk4 - are the 200-type
% files that are the output of HOPS also in a Mk4 format?

\reqid{D} Support \acs{VEX} 2.0 and make it compatible with the
    \acs{VEX2XML} parser tool. \FIX[Does VEX2.0 exist?]
  %\reqex[input required]{Does VEX2.0 exist?}

%% So...does VEX2.0 exist or not?
%it is not clear that \acs{VEX} 2.0 will ever exist, but it does allow the \acs{EHT} 
%to correctly capture its frequency setups and recording allocations.  Such 
%support would need to exist in \acs{DiFX}, and it is not clear that any of its 
%developers care to do this work.
...
\reqex{\FIX[need to elaborate on the requirement that is being tested]}
...
\reqex{\FIX[need to elaborate on this desire]}
...
\reqid{A} There shall be a test to verify \acs{search} functionality
      \reqex{\texttt{chk\_search.sh} currently does a search on a captive
      data set.}
      
\reqid{A} \acs{MHO} shall maintain the \acs{fourmer} tool
...
\FIX[Do we need to support ghoscript?]}
...
\FIX[What does \texttt{chk\_avefix.sh} do? Make sure this test is captured here
and in the coverage and testing doc.]
...
\reqid{A} \ac{MHO} shall have the ability to flag some list of frequency
    intervals.
  \reqex{This is needed when \acs{RFI} or calibration tones signals are
    present.  In \ac{HOPS} \texttt{chk\_notches.sh} verifies this.}
...
% Not sure what's meant by library access here. The functions that HOPS uses
% for LSF should be independently accessible from their library modules?
