...
\item[D], for desires. These are general design goals
%%% EDIT: removed:
%%% EDIT:, which are not
%%% EDIT: esential requirements for HOPS4 and are not directly testable,
%%% EDIT: but are strongly encouraged.
\EDIT[for which testing reports are not required.]
...
\EDIT[Again, no formal test report for these shall be made.]
...
\reqid{A} HOPS4 shall support at least \FIX[128] channels \EDIT[and] should not impose a
...
%%% EDIT: new
\reqid{A} HOPS4 shall support a bit depth of \FIX[2] bits.
  \reqex[input required]{\EDIT[Support for 1-bit is available in HOPS3,
    and hooks for other bit depths will be provided.  But the code for other
    depths will require development.]}
...
\reqex{The \acs{PGPLOT} package has
%%% EDIT: long been obsolete,
\EDIT[out of maintenance for a long time,]
and any replacement
package could follow the same fate. Thus all plotting package calls need to be 
...
%%% EDIT: removed ``be''
\reqid{A} HOPS4 shall support input from the DiFX correlator (in Swinburne format).
...
%%% EDIT: replaced ``This''
  \reqex{\EDIT[Command-line arguments] should be implemented with a
...
%%% EDIT: added:
\EDIT[ but may be adopted at any time]}%
...
in Python in HOPS4 and shall support alist file version %
%%% EDIT: dropped 5 -- at this point that is only for geodesy.
6\footnote{
%%% EDIT: replaced:
%%% 5 \& 6 \footnote{%
...
\EDIT[
Version 6 is the standard for the \ac{EHT}, and a version 7 format seems
...
\reqid{A} The \acs{aedit} program shall process version 6
%%% s5 \& 6
\acs{A-list} 
...
%%% ``there''
\reqid{A} \EDIT{The} current functionality of the program \acs{search} shall be 
preserved.
...
%%% EDIT: changed to D -- either we do or we don't but it's not a test item
%%% RELOCATE below? (D's tend to follow A's).
...
of less data than it should be, leading to incorrect results.
%%% EDIT revised:
\EDIT[This is currently done in HOPS3 provided \acs{DiFX} properly notices
the loss.]}
%%% The ability to flag 
...
list (\eg~of frequency intervals \EDIT[with time in some]) flag file.
...
HOPS3 package.  \EDIT[Here] ``complex'' refers to both amplitude and phase variation 
...
%%% EDIT although we shall most likley do so.}
\EDIT[but it is a capability worth preserving]}.

\EDIT[IT SEEMS TO ME THE FOLLOWING ``SHALLs'' should be ``SHOULDs''.]

\reqid{D} HOPS4 shall preserve the the Single Band Delay, Multi Band Delay 
and Delay Rate search algorithm in its current form.
%%% EDIT: However, in addition it will 
%%% be more modular such that the user may (de)select certain search dimensions.
%%% EDIT: reworded a bit:
\reqex{The current loop in \acs{fourfit} includes a loop over \acs{TEC}
as well \EDIT[which is not needed at \acs{EHT} frequencies], 
which makes 4 loops.  \EDIT[For spectral line data 
\acs{MBD} impossible, so there are two loops too many.
...
\reqid{D} HOPS4 shall support spectral-line VLBI.
\reqex{The existing HOPS code assumes a continuum.}

\reqid{D} HOPS4 shall support pulsar folding with a user specified period or
ephemeris file with blanking.
%\reqex{This may be descoped.}

\reqid{D} HOPS4 shall enable multi-threading or multi-processing for batch jobs.
